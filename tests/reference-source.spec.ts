...
import { fallbackSessionRowsOf, listReferenceRows, type ReferenceMenuResult } from '../src/client/board/reference-source.ts'
...
describe('fallbackSessionRowsOf (宿主会话候选不可用时的板内目录降级)', () => {
  const catalog = [
  ...
  it('excludes the target session (official self rule)', () => {
    const rows = fallbackSessionRowsOf(catalog, 's-target')
  ...
  it('escapes a label like the deployed mention grammar', () => {
    const rows = fallbackSessionRowsOf([{ sessionId: 's1', label: 'a[b]\\c' }], 'target')
  ...
  it('caps at the host candidateLimit default', () => {
    ...
    expect(fallbackSessionRowsOf(many, 's-target').length).toBe(50)
  })

  it('preserves catalog order', () => {
    const rows = fallbackSessionRowsOf([{ sessionId: 'b', label: 'B' }, { sessionId: 'a', label: 'A' }], 'target')
