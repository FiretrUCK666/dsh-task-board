...
import { sessionUnviewedOf, taskUnviewed } from './session-display.ts'
...
/** Whether a settled round is work a person could have to rule on. */
function isCompleted(round: ExecutionRecord): round is ExecutionRecord & { result: 'succeeded' | 'failed'; endedAt: number } {
  return round.endedAt !== undefined && (round.result === 'succeeded' || round.result === 'failed')
}

/** The newest completed round of a set (ties resolve to the later record). */
function newestCompleted(rounds: readonly ExecutionRecord[]): CompletedWork | undefined {
  let best: CompletedWork | undefined
  for (const round of rounds) {
    if (!isCompleted(round)) continue
    if (best !== undefined && round.endedAt <= best.endedAt) continue
    best = { roundId: round.id, sessionId: round.sessionId, result: round.result, endedAt: round.endedAt }
  }
  return best
}

/** The newest finished work on a card, any lane. undefined = nothing decided. */
export function latestCompletedOf(task: TaskRecord): CompletedWork | undefined {
  return newestCompleted(task.executions)
}
 ...
 */
export function sessionGateOf(task: TaskRecord, sessionId: string): HumanGate {
  if (task.status !== 'review') return { state: 'none' }
  const work = newestCompleted(task.executions.filter(round => round.sessionId === sessionId))
  if (work === undefined) return { state: 'none' }
  return {
    state: sessionUnviewedOf(task, sessionId) ? 'unseen' : 'seen',
    work,
    sessionId,
  }
}
 ...
 */
export function gateOf(task: TaskRecord): HumanGate {
  if (task.status !== 'review') return { state: 'none' }
  const sessionIds = new Set<string>()
  for (const round of task.executions) {
    if (round.sessionId !== undefined) sessionIds.add(round.sessionId)
  }
  let seen: HumanGate | undefined
  for (const sessionId of sessionIds) {
    const gate = sessionGateOf(task, sessionId)
    if (gate.state === 'unseen') return gate
    if (gate.state === 'seen' && seen === undefined) seen = gate
  }
  if (seen !== undefined) return seen
  // Session-less legacy rows have no per-session clock; the card's own read
  // baseline is the honest stand-in (the same fallback the notification
  // drawer's legacy lane uses).
  const work = latestCompletedOf(task)
  if (work === undefined) return { state: 'none' }
  return { state: taskUnviewed(task) ? 'unseen' : 'seen', work }
}
...
export function waitingSessionsOf(
  ...
  for (const { sessionId } of relatedSessionIdsOf(task, linkedIdsOf(task))) {
    ...
    seen.add(sessionId)
    // The NUMBERED run, when there is one: `plainRunsOf` is the run sequence,
    // so a waiting comment never shifts the 「第 N 次执行」 a tooltip shows.
    const run = newestRunOf(task, sessionId)
    out.push({
      sessionId,
      waitingKind,
      ...run !== undefined ? { executionId: run.id } : {},
    })
  }
  return out
}

/** The newest plain run of a conversation (the run sequence is plain runs). */
function newestRunOf(task: TaskRecord, sessionId: string): ExecutionRecord | undefined {
  for (let index = task.executions.length - 1; index >= 0; index -= 1) {
    const round = task.executions[index]
    if (round !== undefined && round.sessionId === sessionId && round.comment === undefined) return round
  }
  return undefined
}
