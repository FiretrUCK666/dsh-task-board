...
export interface FeedRowProps {
  ...
  status: { kind: ChipKind; label: string }
...
export function FeedRow({
  ...
  mainControls,
  actions,
}: FeedRowProps): ReactNode {
  return (
    <div className={css.notifyRow}>
      <button
        type="button"
        className={css.notifyMain}
        {...(mainTitle !== undefined ? { title: mainTitle } : {})}
        {...(mainExpanded !== undefined ? { 'aria-expanded': mainExpanded } : {})}
        {...(mainControls !== undefined ? { 'aria-controls': mainControls } : {})}
        onClick={onMainClick}
      >
        {task !== undefined && <span className={css.notifyTask}>{task}</span>}
        {session !== undefined && (
          <span className={css.notifySession} {...(sessionTitle !== undefined ? { title: sessionTitle } : {})}>
            {session}
          </span>
        )}
        <Chip kind={status.kind} fill={false}>{status.label}</Chip>
      </button>
      {excerpt !== undefined && excerpt !== '' && (
        <p className={css.notifyExcerpt}>{excerpt}</p>
      )}
      <span className={css.notifyActions}>
        <span className={css.notifyActionCluster}>{actions}</span>
        <span className={css.notifyMeta}>{time}</span>
      </span>
    </div>
  )
}
