...
*/
function volatileValue(field, fallback) {
	if (field === void 0 || field === null) return fallback;
	const candidate = field;
	const current = typeof candidate.get === "function" ? candidate.get() : field;
...
function apply(ctx, config) {
	...
	const sync = () => {
		...
		if (!volatileValue(current().enabled, DEFAULT_ENABLED) || !volatileValue(current().announceToAgent, DEFAULT_ANNOUNCE)) return;
	...
	ctx.inject(["settings"], (child) => {
		child.effect(() => child.settings.configure({ auto: false }, ctx.fiber), "dsh-task-board: settings page policy");
	});
...
//#endregion
export { ConfigSchema, TASK_BOARD_GUIDANCE, TASK_BOARD_SETTINGS_NAMESPACE, apply, inject };
