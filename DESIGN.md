...
description: DSH Web 界面里的任务看板插件——借来的色阶、密集的 11-13px 层级、全药丸形状，靠状态呼吸而不是靠装饰说话。
colors:
...
6779 行样式里没有一处硬编码颜色（由 `pnpm verify` 的审计强制），所有颜色、边框、阴影、
文本层级与动效常量都从一层别名流向宿主语义令牌。这带来一个罕见的结果：换主题、换皮肤时
这块板自动跟着走，零适配。视觉个性的位置因此不在调色板上，而在**几何与状态的精确度**上。

第二原则是**密集但可读**。这是操作界面（Operate），不是营销页：字号落在 11–13px 区间，
按钮 28px 高，卡片内边距 9/12px，节奏靠具名 gap 而不是继承的边距。密度换来的是一屏能装
下更多真实信息；可读性则靠三档明确的文本层级与永不重叠的具名网格来守住，而不是靠放大字号。
...
其余动效一律短促（160ms）、有目的、可降级。

**Key Characteristics:**
- 颜色完全借自宿主令牌，板零所有权，随主题与皮肤自动漂移
- 密集的 11/12/13px 三档层级，靠层级与对齐而非字号制造清晰
...
- **Ink / Ink Soft / Ink Faint** (`#0f1115` / `#61666b` / `#81858c`；暗色 `#f9fafb` / `#cfd3d6` / `#adb2b8`)：
  三档文本层级。第三档只用于真正可舍弃的元信息（时间、提示、占位符）。
...
- **Title** (600, 16px, 24px)：弹窗标题与详情页主标题。这是唯一越过 13px 的层级，
  用于「你正在处理一整件事」的场合。
- **Control** (500, 13px, 1)：按钮、输入框、下拉框、次级标题。字重 500 是这里的默认强调方式——
  比加粗克制，比常规可读。
- **Body / Row line** (400–500, 12px, 1.5 或声明的 20px 行盒)：界面绝对主力（82 处）。
  卡片标题、会话名、说明文字、列表行都在这一档。
- **Meta** (400, 11px, 16px)：时间戳、路径、代码块、最小的辅助标记（18 处）。
- 外壳的完整阶梯（24/20/16/14/13/12/11）是可继承的上游参照，但板自己只用到最下面三段——
  这块界面要在一屏里装下更多真实信息。

### Named Rules

**The Three-Size Rule.** 界面文字只用 11 / 12 / 13px 三档（外加 16px 的标题档）。
...
- **紧凑档（< 680px）**：五列变成一个横向 `scroll-snap` 轨道（每列宽到能读），配一排
  五等分的列导航标签（短名 + `aria-label` 全名）；板头换行成确定的两行；底部出现拇指栏
...
- **卡片与面板走圆角阶梯，且与尺寸成正比**：卡片 14px（`radius-md`）、列与浮起面板 20px
  ...
  从属关系由几何本身就说明白。
- **非药丸的小物件保留自己的小半径**：仪表色块、`code`、删除 ×（3–4px）、圆形徽标（50%）。
  它们不是按钮，不该被药丸语言同化。
