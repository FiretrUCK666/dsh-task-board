...
（`task.status === 'running'`，与黄边 `data-status` 是同一个事实）以及等待 / 进行中 / 完善中；
...
**边缘**有起伏。所以 halo 的峰值 = 内晕 + 柔外沿（spread 令牌、软档），ring 仍是纯外凸强档；
两种形态依旧分明（inset 归 halo、强度分档），`card-contract.spec` 钉住峰值形状。
...
- **状态词表一处派生**（`noteStatusShapeOf`）：权限审批 / 计划确认 / 提问（琥珀）、
  ...
  待决策（红）、已取消（灰）；动态行的词表是另一张纯表（`activityChipOf`），两表都只回答
  各自「发生了什么」。
- **计数单位统一为「行」**：铃徽标 = 未见点 = 抽屉行数，点开即所见；需求行（等你处理/待审核）
  保持自己的语义（等待按会话、待审按任务），互不争论。
- **动态屉的层级**：**日标题 → 任务折（标题 ×N · 最新时间 · chevron，单开、开屉自动展开
  ...
  plain）都算一条道，refine 永远不算（完善是准备，不是门里的车道）。

### 交互卡（Interaction Card）

提问卡与计划卡共用一副骨架，两条律法管住它们的「桌面截断」史：

- **高度契约**：卡至多 `min(320px, 100%)` 高，正文内滚、头尾钉住——长计划/长题永远推不走
  确认按钮与输入框。
