...
两个抽屉的每一行是同一个构件（`.notifyRow`），一会话一行，不按任务折叠：

- **身份行顺序 = [任务][会话][状态]**：任务和产出它的会话读作一个单元，状态紧挨着它解释的
  动作；任务名弹性伸缩、两个标题各有省略地板（6em / 4em），状态 chip `flex: none` 永不被压。
...
- **`.notifyMain` 带最小宽度地板**（`min(100%, 16rem)`）：身份行装不下时动作簇整组**换行**，
  而不是把标题挤成细缝——让位，不压扁。
- **状态词表一处派生**（`noteStatusShapeOf`）：权限审批 / 计划确认 / 提问 / 待审核 / 待决策 /
  已取消；动态行的词表是另一张纯表（`activityChipOf`），两表都只回答各自「发生了什么」。
