...
- **One set of status words** — Many places on the board say "what state is this card in". They all read one derivation, so the same card gives the same answer everywhere:
  - **Awaiting your decision** (reads *Failed · awaiting your decision* when the work failed) — the card is in Needs review, has finished work, and you have not looked at it yet. It clears when you open the card or the session, or when you approve or send it back.
  ...
  - **M in review** (the header line) — M cards are in Needs review with results you have not looked at. Same clearing as the chip.
  - **New N / New comment** — there is content you have not read. It clears on opening.
  - **N runs** — how many times the card has run. That is history, not a request, so it never clears on its own.
  - Two things worth knowing: *New* and *Awaiting your decision* are different — "you have not read it" versus "you have neither read it nor ruled on it". And a **cancelled** run is not a decision, so it never asks for one; it only appears in the comment thread and the activity feed.
