...
- **自动执行（定时 / 接续）**：详情面板可为任务配置两种驱动模式，规则有统一的生命周期，界面信息永远真实（不显示「下次运行」时必有原因）：
  ...
  - 两种模式共用「运行次数上限」（留空 = 无限）；手动执行/拖拽不消耗自动运行计数，
    与自动规则互不干扰。
- **评论与评论页**：点执行记录某一条打开居中评论页——左侧显示该次执行会话的**最后一段对话**（视觉对齐原生，只显示尾部；完整历史用「查看会话」跳原生会话页，两页职责不同：评论页回顾 + 干预，会话页完整交互）；右侧栏分三段：**固定头部**（上下文用量条 + 会话面板 + 会话事实，永远可见、不会被评论挤走）→ **独立滚动的评论区** → **发送区固定栏底**。**会话面板**：模型/思考程度（`sessions.models`/`selectModel`）与权限（原生 `/permission` 命令注册表执行——**不是**把命令文本当消息发给 agent，所以切权限绝不会产生 agent 的「无法更改」式回复）即时作用于当前会话；权限下拉以原生 `permissions` 投影为权威（`currentValue`/`options`，与原生 PermissionSelect 同源，切换成功立即刷新同步、轮询兜底）；会话工作区（cwd）与实际组合 Agent 只读展示（会话已运行无法换 Agent——原生 `agent-preset-locked`，任务卡片编辑的 Agent 作用于下次新执行，两处互不影响）。**上下文用量条**：原生 ContextMeter 同源同算法（`contextPressure`/`contextBreakdown` 投影，bluish-400 / #a78bfa / blue-450 三色段，无投影时降级 tokens 汇总）。**评论区是每任务 FIFO 队列的跨执行全量线程**：任务的全部评论历史跨执行/跨会话一条不丢，每条带「第 N 次执行」序号徽章（对应所属执行，找不到则显示「未知执行」）；可连续保存多条；巡航关 =「已保存」（可取消）、预算满/任务忙 =「排队中 · 第 N 位」（可取消）、注入后运行中、结算后显示结果；卡片同步「评论排队 N」徽章。**评论以 `/` 开头 = 斜杠命令**（与原生产法完全一致）：经原生命令注册表执行（如 `/plan ...` 真的会进入 plan 模式、`/permission ...` 真的切权限），未知命令退回普通文本（不丢输入），命令结果文案显示在评论条目下。**后台交互可见**：会话等待你处理（审批/plan/提问）时显示「等待回应」横幅 + 卡片徽章。**对话实时同步**：3 秒轻轮询按投影水位（asOfSeq）门控——只有会话真的产生新事件才重渲染（行级 memo，长对话也不卡），标签页隐藏时暂停、回来自动补刷，另有手动「刷新」；新输出贴底自动跟随滚动（主动上翻则不打扰，回到底部后恢复）。窄屏（<720px）回退单列堆叠。对话区按原生会话风格渲染，顶部结果横幅。
