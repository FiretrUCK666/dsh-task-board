...
- **拖拽**：卡片拖到其它列 = 移动（拖到「进行中」= 立即重新执行）；同列内拖动 = 调整顺序，插入位置有精确指示条；执行中拖到「待审核/已完成」会被拒绝并红闪提示；拖放成功后落点列有一次确认闪烁。侧边栏的**单个会话或整个工作区文件夹也可以直接拖进来建卡**（目标列呼吸高亮），拖到哪列就落在哪列。
...
- **链接会话**：拖入的会话/工作区以**实时视图**展示（新开会话、归档、改名自动同步，无需手动刷新）。点开会话面板——与评论页同一套体验（对话 + 上下文条 + 可改配置 + 会话事实 + 该会话自己的评论线程）。底部发送区有两个显式模式（默认「驱动任务」，一键切换）：「驱动任务」的留言=给该会话留评论，进入任务评论队列按序注入（与执行评论页同一条队列、同一调度），让任务继续跑；「直发会话」=直接向该原生会话发消息（等于在原生对话里打字，**不会**驱动本任务）。没有执行记录的任务也能直接在链接会话面板用「驱动任务」留言开跑；任务已完成时两模式都拒发并提示先移回待办。支持隐藏与解绑。
...
11. 链接会话与留言驱动：把侧边栏一个**会话**（或工作区文件夹）拖进看板 → 出现链接会话行
    （拖到哪列就建在哪列）；打开该会话面板，默认「驱动任务」——输入一句留言并发送 → 卡片
    出现「评论排队 N」→ 开启自动巡航（或已有空闲槽位）→ 该留言注入会话（卡片变「进行中」，
    会话里真出现这句话），agent 跑完落「待审核」，面板评论线程显示成功；排队中可随时取消。
    切到「直发会话」再发一句 → 消息直接进原生会话，卡片状态不动。任务已完成时发送被拒并
    提示先移回待办。
