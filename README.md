...
- **拖入建卡（链接会话）**：把侧边栏工作区里的**单个会话**或**整个工作区文件夹**直接拖进看板，拖到哪列就落在哪列（五列都尊重）→ 立即生成一张新卡片。卡片会出现独立的**「链接会话」**区：它是该工作区/会话的**实时视图**（原生数据、非副本）——新开会话、归档、改名都会自动同步到这里，因此无需手动「同步」，只有隐藏过会话时才出现「恢复全部已隐藏」；每条可「查看会话」，已归档的不显示，「隐藏」单条仅不显示（不归档不删除，编号稳定）；**点击会话行打开会话详情面板**（与执行评论页同一外壳：实时对话 + 会话事实 + 查看/隐藏/解绑，**只读**——在此留言不会驱动任务，驱动请用执行记录评论页）。卡片的描述、执行 Prompt、运行配置照常填写，只作用于板内发起的执行，不影响这些外部会话；「解绑」可转回普通任务。执行记录每条同样可「隐藏」不删除。
