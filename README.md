...
- **多列看板**：待规划 / 待办 / 进行中 / 已完成 / 已失败 五列；卡片显示标题、描述、
  工作区、更新时间与执行次数（执行中显示「进行中 · 第 N 次执行」）；顶部支持搜索过滤、
  新建任务，「返回对话」固定在右上角。
- **任务详情**：点卡片打开详情（标题/描述/执行 Prompt/运行配置/执行记录），**不会**一点
  就执行；运行配置以标签-值网格列出工作区、Agent、模型、思考程度、权限；执行记录按
  「第 N 次执行」列出精确到秒的开始/结束时间与耗时；详情内提供「执行 / 重新执行」
  「删除（带确认）」「查看会话（跳转到执行 transcript）」以及手动移到待规划/待办。
