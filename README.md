...
- **定时任务**：详情面板可为任务配置定时执行——启用开关 + 5 段 cron 表达式
  （分 时 日 月 周，支持 `*` / `*/n` / `a-b` / 逗号列表）+ 19 个常用预设（按
  常用频率 / 每天 / 每周 / 每月 分组），cron 输入框下方**实时显示人类可读描述**
  （如「每 10 分钟」「每周一 09:00」）；启用即计算并持久化「下次运行时间」，卡片
  显示 定时 标识；到点自动走真实执行链路（同手动执行），执行会话照常可跳转。
...
8. 刷新页面/重启 DSH → 任务仍在；卸载插件 → GUI 恢复原状。
