...
入口和 DSH 自带的「插件」面板并排：点它，看板占用中间主区域；再点一次同一个入口、或点侧边栏里的任意会话，即可回到对话。

这个插件**只有一个开关：开或关**。位置有两处，做的是同一件事：侧边栏「插件」→ 往下拉到「已安装」，那一行右边的开关；或点开「任务看板」进它自己的页面，右上角那个开关。**关掉 = 插件不加载**（侧边栏入口、看板、后台接口一起停），**打开 = 恢复**。

### 不需要手动配置任何东西

装完就能用，不用改配置文件、不用手写任何声明。而且**不要去改 `cordis.patch.yml`**——插件那一行住在插件包里，安装命令只负责把这个包登记进 profile，两者一配合就装配好了；手工再补一行会造成同一个插件出现两条。
...
这个插件没有设置项，也不需要设置面板：看板的行为（任务、定时、巡航、规则）全在看板界面上直接编辑，而开与关就是上面那个开关。
...
卡片换栏时自动顶到目标栏最上方（按发生时间，最新完成的在最上），所以「刚完成的是哪一张」一眼可见；你手动拖过的位置不会被顶掉，只有它下次换栏时才重新排。

卡片里的会话列用同一条律：某个会话**开始工作或刚跑完**时，它会顶到会话列表最上方，其余会话保持相对次序往下让位；你手动拖过的会话次序同样不会被顶乱。

#### 板上的状态词

板上很多地方都在说「这张卡现在怎么样」，它们读的是**同一份推导**，所以同一个卡片在任何一处
都是同一个答案：

| 你看到的词 | 它在说什么 | 什么时候消失 |
| --- | --- | --- |
| **待你决断**（失败时写「失败待决断」） | 这张卡在「待审核」列，有已经跑完、但那个**会话**你还没看过的结果 | 你打开那个会话的评论区，或者点「通过 / 打回」 |
| **等你处理 N 项**（板顶那行） | 有 N 个会话正挂起等你回答（权限审批 / 计划确认 / 提问） | 你在那个会话里回答完 |
| **待审核 M 张**（板顶那行） | 有 M 张卡在「待审核」列，且那个会话你还没看过 | 同「待你决断」 |
| **新 N / 新留言** | 这张卡有还没看过的**新内容** | 点开卡片即消失 |
| **待处理 N** | 有 N 个会话在等你（多个时 N > 1） | 逐个回答完 |
| **N 次执行** | 这张卡跑过几轮（这是历史，不是催你） | 不会自己消失 |
| **失败 · 已暂停** | 自动化因为上一次跑失败而暂停 | 你在详情里重新启用 |

**看板分两层，这是有意设计的**：**卡片是摘要，会话是内容**。所以

- 点开**卡片** → 卡片的「新 N」和闪烁消失（你看过摘要了），但「待你决断」和通知**留着**——
  因为你还没看过真正的那段对话。
- 点进**那个会话的评论区** → 「待你决断」和通知才一起消失。
- 或者直接点「通过 / 打回」→ 卡片移走，提醒消失。

通知中心里的东西只有两类，三颗筛选芯片各写着数：**等你处理**（会话挂起等你作答）+
**待审核**（跑完了、那个会话你还没看过）= **全部**。铃铛上的数、板顶那行的两半、抽屉里的三颗
芯片，是同一个分类的四处渲染，永远对得上。

两点值得知道：

- **「待你决断」和「新 N」是两件事**。「新」是「这张卡你没打开过」，「待你决断」是「那个会话
  你没看过、也还没裁决」。
- **取消不算「待你决断」**。取消是中止，不是等你裁决的事；它只出现在评论线程和动态里。

**会话建卡**：板头的「会话建卡」按钮挑几个已有会话（可跨工作区多选），直接造一张带着这些对话的新卡片，落在「待规划」，标题/描述/Prompt/运行配置全部留空待你填。任务详情里的「添加会话」与它共用同一个面板——按工作区分组、默认全折叠、只列未归档且可添加的会话。
...
设置项从「设置 → 任务看板」搬到**插件自己的页面**（侧边栏「插件」→ 已安装 → 点「任务看板」）——
开关和插件本体放在同一页。功能没有减少，任务数据也没有变化。
...
本插件没有设置项，所以也没有设置面板：插件市场里那条开关写的是 profile 行的 `disabled`（关掉时才写上），插件本身不声明任何设置字段。
