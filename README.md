...
- **自动执行（定时 / 接续）**：详情面板可为任务配置两种驱动模式，规则有统一的生命周期，界面信息永远真实（不显示「下次运行」时必有原因）：
  ...
  - 两种模式共用「运行次数上限」（留空 = 无限）；手动执行/拖拽不消耗自动运行计数，
    与自动规则互不干扰。
- **评论与评论页**：点执行记录某一条打开居中评论页——左侧显示该次执行会话的**最后一段对话**（视觉对齐原生，只显示尾部；完整历史用「查看会话」跳原生会话页，两页职责不同：评论页回顾 + 干预，会话页完整交互）；右侧是**独立滚动栏**（发送区固定在栏底）：**上下文用量条**（原生 ContextMeter 同源同算法——「上下文已用 X% · ~N / M」+ 系统提示词 / 工具 / 对话消息三段分色长条 bluish-400 / #a78bfa / blue-450，数据来自 history 尾页的 `contextPressure`/`contextBreakdown` 投影；无投影时降级为 tokens 汇总）、**会话面板**（一行紧凑网格：模型/思考程度（`sessions.models`/`selectModel`）、权限（原生 `/permission` 路径）**即时作用于当前会话**；Agent 选择器作用于**下次新执行**——会话已运行则无法中途换 Agent（原生 `agent-preset-locked` 限制，界面明示）；其下只读列出该会话真实工作区（cwd）与实际组合 Agent，与任务卡片配置互不影响）、**评论区**。评论是**每任务 FIFO 队列**：可连续保存多条；巡航关闭时显示「已保存」（不注入、可取消），巡航开启但并发预算满/任务忙时显示「排队中 · 第 N 位」（可取消），注入后显示运行中，结算后显示结果——卡片上同步出现「评论排队 N」徽章。评论输入框与执行 Prompt 共用同一套**斜杠命令菜单**。**后台交互可见**：会话等待你处理（权限审批 / plan 确认 / 提问）时评论页显示「等待回应」横幅、卡片同步「等待回应 · 类型」徽章，点「查看会话」前往处理。数据打开期间每 10 秒自动同步 + 手动「刷新」。窄屏（<720px）自动回退单列堆叠，功能零删减。对话区按原生会话风格渲染（用户右侧气泡 / 助手全宽通栏 / 上下文注入小横条），顶部结果横幅。评论不占执行记录列表。
...
4. 点卡片 → 详情可见内容与 Prompt；点「执行」→ 卡片变「进行中」（会话列表出现
   ...
   跳转到真实 transcript。点执行记录那条 → 打开评论页：左侧是该次会话的最后对话，右侧栏可留评论（巡航未开时显示「已保存」，可连续保存多条；开启巡航后按并发预算排队注入，可取消未注入的评论）；「移到已完成」按钮确认收尾。
