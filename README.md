...
- **多列看板**：待规划 / 待办 / 进行中 / 已完成 / 已失败 五列；卡片显示标题、描述、
  ...
  同一套语义色文字、与标题左缘严格对齐，窄列自动换行不溢出；**卡片可直接拖拽到任意
  列**（拖到「进行中」= 立即重新执行；执行中拖到「已完成/已失败」会被拒绝并红闪提示，
  与定时执行/结算逻辑互不冲突）；顶部支持搜索过滤、新建任务，「返回对话」固定在
  右上角。
...
- **定时任务**：详情面板可为任务配置两种驱动模式：
  ...
  - 两种模式共用「运行次数上限」（留空 = 无限）；手动执行/拖拽不消耗定时计数，
    与定时规则互不干扰。
...
6. 定时任务：详情 →「定时运行」勾选启用，选预设「每 10 分钟」（cron `*/10 * * * *`），
   卡片出现 定时 标识；等待下一个整 10 分钟点，观察卡片自动进入「进行中」并最终完成，
   详情「上次触发」出现时间、执行记录新增一条（会话可跳转）。编辑过 Prompt 的任务，
   定时触发的下一次运行同样使用最新内容。「管理预设」里可增删改自定义预设、一键恢复
   默认。
7. 拖拽：按住卡片拖到「待规划/待办/已完成/已失败」→ 卡片移动；拖到「进行中」→
   任务立即重新执行（正在执行时拖到「进行中/已完成/已失败」会被拒绝，列边框红闪）。
8. 完成后接续：详情「定时运行」→ 模式切到「完成后接续」，留空次数 = 无限——任务
   立即开始执行，每次完成后自动续跑，卡片持续「进行中」；设次数上限则跑满后停止并
   落「已完成」。
