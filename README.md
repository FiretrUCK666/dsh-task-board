...
- **拖拽**：卡片拖到其它列 = 移动（拖到「进行中」= 立即重新执行）；同列内拖动 = 调整顺序，插入位置有精确指示条；执行中拖到「待审核/已完成」会被拒绝并红闪提示；拖放成功后落点列有一次确认闪烁。自动化**不再把卡片绑死**——接续中的卡片照常可拖，拖到「已完成」= 停链、拖到「待规划/待审核」= 暂停、拖到「待办」= 停止接续手动接管。侧边栏的**单个会话或整个工作区文件夹也可以直接拖进来建卡**（目标列呼吸高亮），拖到哪列就落在哪列。
...
- **执行记录**：每次执行列出精确的开始/结束时间与耗时、成功/失败；行身份 = 该次会话标题（安静注记「第 N 次」），两族会话行共用同一行文法；行内直接显示该次执行的最新评论与等待状态；可隐藏某条（不删除、编号不变）。
- **评论页**：点执行记录打开——左侧是该次会话的实时对话（尾部），右侧是上下文用量条 + 会话配置 + 评论区 + 固定发送区。**留言会驱动任务继续跑**（按提交顺序排队注入，显示真实位次；`/` 开头的斜杠命令走原生命令注册表）；会话的模型 / 思考程度 / 权限可在此即时调整。
- **链接会话**：拖入的会话/工作区以**实时视图**展示（新开会话、归档、改名自动同步，无需手动刷新）。点开会话面板——与评论页同一套体验（对话 + 上下文条 + 可改配置 + 会话事实 + 该会话自己的评论线程）。底部发送区有两个显式模式（默认「驱动任务」，一键切换）：「驱动任务」的留言=给该会话留评论，进入任务评论队列按序注入（与执行评论页同一条队列、同一调度），让任务继续跑；「直发会话」=直接向该原生会话发消息（等于在原生对话里打字，**不会**驱动本任务）。
- **评论按会话统一**：执行记录与链接会话**共用同一评论线程**——同一个原生会话里，从执行页留的评论和从链接面板留的评论互见同一列表，不再各看各的；未注入的评论保留「已保存/排队中」状态可取消。没有执行记录的任务也能直接在链接会话面板用「驱动任务」留言开跑；任务已完成时两模式都拒发并提示先移回待办。支持隐藏与解绑。
- **未查看提醒**：有内容没看过时，卡片带呼吸光晕 + 「新 N」徽章；执行行有未读圆点，点开即清；旧数据升级不闪屏。

### 自动化

- **自动化（任务级）**：详情里可为任务配置**定时执行**（5 段 cron + 内置预设）或**完成后接续**（可设次数上限或无限续跑）。**开启即生效**——定时到点自动触发、接续立即开始第一轮，无需先手动执行；任务移到「已完成」即自动停用，移到「待审核/待规划」时暂停（失败会标注「上次自动运行失败」原因）。编辑器提供「跳过本次」（定时）、「停止接续」（接续，带确认）干预；无限接续启用前有一次确认。
...
3. 「+ 新建任务」填标题/描述/Prompt（Prompt 输入框内输入 `/` 会弹出命令菜单，可选中
   插入命令）；「初始状态」可选「待规划 / 待办」（默认待办）→ 卡片出现在所选列。检查：
   新建任务弹窗**在看板区域内居中**（不贴列线、不偏右）。
   待规划任务可在详情页「需求完善」里让 AI 调研并打磨出可执行的 Prompt。
4. 点卡片 → 详情在看板内居中。执行记录与链接会话统一在一个「会话」section：执行行身份=
   该次会话标题 + 安静「第 N 次」注记，「查看会话」是紧凑小按钮。点「执行」→ 卡片变
   「进行中」；agent 跑完后落「待审核」，行内直接显示最新评论与等待状态，可「查看会话」
   跳转真实 transcript。「自动化」模块默认折叠为一行**三态摘要**（未启用 / 已暂停(原因) /
   运行中），展开为完整编辑器。点执行记录那条 → 评论页（对话 + 上下文条 + 配置 + 评论区 +
   固定发送区；巡航未开时留言显示「已保存」，开启后按并发预算排队注入，可取消未注入的）。
...
7. 拖拽：卡片拖到其它列 → 移动（拖到「进行中」→ 立即重新执行；执行中拖到「待审核/已完成」
   红闪拒绝）；同列拖动 → 调整顺序（精确指示条）。**接续中的卡片不再被绑死**：拖到
   「已完成」= 停链；「待规划/待审核」= 暂停；「待办」= 停止接续手动接管。卡片**悬停浮现
   ▶ 快速执行**按钮。侧边栏会话/工作区拖进看板可建链接任务卡。
