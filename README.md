...
插件的开关在**侧边栏「插件」→ 往下拉到「已安装」→ 点「任务看板」**，那条插件自己的页面里：勾选就是显示，取消就不显示。
...
**设置项只有一个开关，默认就是开的**，不填也照常工作：关掉它，看板就不显示；再打开，就回来。**改完立即生效，不用刷新页面、也不用重启**。它和同一个页面上那个（右上角的）启用开关不是一回事——那个是连插件一起卸下（接口也不再提供），这个只是把看板从屏幕上收起来，实时执行与定时照常在后台跑。
...
设置项没有自己的 slot：插件在 `Config` schema 里声明字段，侧边栏「插件」里这条插件自己的页面（已安装 → 点插件名）把它渲染成表单，开关就长在那里。
