...
- **自动执行（定时 / 接续）**：详情面板可为任务配置两种驱动模式——**手动首启**：只有被手动「执行」（或拖到「进行中」）启动过的任务，自动规则才会接管；尚未手动启动的任务，即使启用规则也绝不会被自动执行：
...
- **自动执行在浏览器端调度**：插件是纯客户端（无服务端通道），所以「到点执行」由
  ...
  页面加载早期（会话列表基线未就绪）不触发，避免误执行。**手动首启**：启用自动规则本身从不执行任何东西——只有被手动「执行」（或拖到「进行中」）启动过的任务，其规则才会被激活（primed），此后到点触发与接续才生效；未手动启动任务的规则只是「待命」。限制：需要标签页保持打开
...
6. 自动执行：详情 →「自动执行」勾选启用，选预设「每 10 分钟」（cron `*/10 * * * *`），
   ...
   运行同样使用最新内容。「管理预设」里可增删改自定义预设、一键恢复默认。
