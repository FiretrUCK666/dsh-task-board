...
- **评论驱动**：点某条会话行打开对应面板——左侧是该会话的实时对话（尾部），右侧是上下文用量条 + 会话配置 + 评论区 + 固定发送区。**留言会驱动任务继续跑**（按提交顺序排队注入，显示真实位次；`/` 开头的斜杠命令走原生命令注册表）；模型 / 思考程度 / 权限可在此即时调整。链接面板另有「直发会话」模式 = 直接向该原生会话发消息（等于在原生对话里打字，**不会**驱动本任务）；直发消息同样**落进该会话的评论线程**（带「直发」标记），与驱动评论同线程可见。
