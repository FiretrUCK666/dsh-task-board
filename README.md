...
- **看板头部是一体化工具栏**：左侧标题块 + 右侧统一 28px 胶囊动作簇（胶囊搜索、新建任务、自动巡航开关+并发、返回对话图标），自动巡航是透明行内控件（不再是白底盒），玻璃皮肤下不露丑。
- **拖拽**：卡片拖到其它列 = 移动（拖到「进行中」= 立即重新执行）；**同列内拖动 = 调整顺序，上拖/下拖/中间穿插都准（插入点精确指示条）**；执行中拖到「待审核/已完成」会被拒绝并红闪提示；拖放成功后落点列有一次确认闪烁。自动化**不再把卡片绑死**——接续中的卡片照常可拖，拖到「已完成」= 停链、拖到「待规划/待审核」= 暂停、拖到「待办」= 停止接续手动接管。侧边栏的**单个会话或整个工作区文件夹也可以直接拖进来建卡**（目标列呼吸高亮），拖到哪列就落在哪列。
...
- **统一「会话」模型**：执行记录（板内执行的会话）与链接会话（拖入绑定的外部会话）合并为一个**按 sessionId 去重的「会话」列表**——同一会话绝不会出现两次，从执行页或链接面板进入同一会话看到的是**同一条评论线程**（评论按会话统一，结构性解决"评论区不同步"）。每个会话行统一文法：会话标题/工作区标签 + 实时状态 chip + 安静「第 N 次」注记（执行行）+ 评论摘要 + 等待状态；可隐藏（按会话，不删除、编号不变）、恢复全部。没有执行记录的任务也能直接在链接会话面板用「驱动任务」留言开跑；任务已完成时两模式都拒发并提示先移回待办。
- **评论驱动**：点某条会话行打开对应面板——左侧是该会话的实时对话（尾部），右侧是上下文用量条 + 会话配置 + 评论区 + 固定发送区。**留言会驱动任务继续跑**（按提交顺序排队注入，显示真实位次；`/` 开头的斜杠命令走原生命令注册表）；模型 / 思考程度 / 权限可在此即时调整。链接面板另有「直发会话」模式 = 直接向该原生会话发消息（等于在原生对话里打字，**不会**驱动本任务）。
- **小功能**：详情页可**「复制为模板」**（以当前内容新建一张待规划卡，执行记录/自动化/链接不复制）；执行 Prompt 旁有**「复制」**按钮一键取用。
- **未查看提醒**：有内容没看过时，卡片带呼吸光晕 + 「新 N」徽章；执行行有未读圆点，点开即清；旧数据升级不闪屏。

### 自动化

- **自动化（任务级，独立分区）**：详情页以一行状态摘要折叠呈现（未启用 / 运行中·下次或第 N 次 / 已暂停·原因），展开才是编辑器（触发方式分段 + 定时或接续配置 + 按状态显隐的「跳过本次/停止接续」），无保存/取消（即改即生效）；与「自动巡航」的关系一句话讲清（本任务自动化只管本任务；巡航在板级批量调度全部任务，互不覆盖）。**开启即生效**——定时到点自动触发、接续立即开始第一轮，无需先手动执行；任务移到「已完成」即自动停用，移到「待审核/待规划」时暂停（失败标「失败·已暂停」）；无限接续启用前有一次确认。
- **自动巡航（板级）**：看板头一键开启（透明行内开关），批量执行全部待办任务；并发上限是全局的——手动、定时、接续、评论留言都走同一个调度器，满额自动排队、释放即补位。
...
4. 点卡片 → 详情在看板内居中。执行记录与链接会话统一为**一个按 sessionId 去重的「会话」
   ...
   **三态摘要**（未启用 / 已暂停(原因) / 运行中），展开为完整编辑器。详情页有**「复制为模板」**
   （新建同款待规划卡）与 prompt 旁的**「复制」**按钮。点某个会话行 → 打开该会话面板
   （对话 + 上下文条 + 配置 + 评论区 + 固定发送区；巡航未开时留言显示「已保存」，开启后
   按并发预算排队注入，可取消未注入的）；执行行打开执行页、链接行打开链接面板。
5. 点详情「编辑」修改标题/描述/Prompt（或运行配置）→ 保存后卡片立即更新；再点
   「执行」，会话以新标题命名、发送的是新 Prompt。
6. 自动化（定时）：详情 →「自动化」勾选启用（**开启即生效，无需先手动执行**），选预设
   「每 10 分钟」（cron `*/10 * * * *`），卡片出现「定时」标识；到下一个整 10 分钟点，
   卡片自动进入「进行中」并最终落「待审核」。详情「下次运行」实时显示；点「跳过本次」
   可把下次运行顺延到下一个匹配时刻。失败后落「待审核」并暂停（摘要显示「已暂停 · 失败」、
   卡片显示「失败 · 已暂停」），到点不复活——「重新执行」或移到待办即恢复。
