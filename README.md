...
- **多列看板**：待规划 / 待办 / 进行中 / 已完成 / 已失败 五列；卡片显示标题、描述、
  ...
  同一套语义色文字、与标题左缘严格对齐，窄列自动换行不溢出；**卡片可直接拖拽**：
  拖到其它列 = 移动（拖到「进行中」= 立即重新执行），**同列内拖拽 = 调整卡片顺序**
  （悬停卡片上方出现插入指示条，松手即插入该位置）；执行中拖到「已完成/已失败」会
  被拒绝并红闪提示，与自动执行/结算逻辑互不冲突；顶部支持搜索过滤、新建任务，
  「返回对话」固定在右上角。
...
- **自动执行（定时 / 接续）**：详情面板可为任务配置两种驱动模式，规则有统一的生命周期，界面信息永远真实（不显示「下次运行」时必有原因）：
  ...
  - **暂停**：任务已手动启动过，但当前处于**待规划 / 已失败**——自动执行暂停，界面提示「已暂停」；任何让任务离开这两个状态的手动动作（执行/重新执行、拖到「进行中」、移到待办/已完成）都会恢复，不依赖唯一路径。
...
- **自动执行在浏览器端调度**：插件是纯客户端（无服务端通道），所以「到点执行」由
  ...
  页面加载早期（会话列表基线未就绪）不触发，避免误执行。**规则生命周期**：scheduler、controller 与 UI 共用同一个判定（见 tasks.ts 的 ruleReadiness）——待命（启用但未手动启动）绝不触发；生效（已启动且处于待办/进行中/已完成）到点触发与接续正常；暂停（已启动但处于待规划/已失败）到点跳过并顺延，只在手动恢复（执行、移到待办等）后继续。限制：需要标签页保持打开
...
4. 点卡片 → 详情可见内容与 Prompt；点「执行」→ 卡片变「进行中」（会话列表出现
   以任务标题命名的会话，卡片显示「进行中 · 第 1 次执行」）；agent 跑完后卡片落
   「已完成」或「已失败」，详情执行记录按「第 1 次执行」列出精确的开始/结束时间与
   耗时，可「查看会话」跳转到真实 transcript。
...
6. 自动执行：详情 →「自动执行」勾选启用，选预设「每 10 分钟」（cron `*/10 * * * *`），
   ...
   运行同样使用最新内容。「管理预设」里可增删改自定义预设、一键恢复默认。任务失败落「已失败」后，自动执行暂停（详情显示「已暂停」，不再显示下次运行），到点不会复活——点「重新执行」或先「移到待办」即恢复。
7. 拖拽：按住卡片拖到其它列 → 卡片移动（拖到「进行中」→ 任务立即重新执行；正在执行时
   拖到「进行中/已完成/已失败」会被拒绝，列边框红闪）；**同列内拖动 → 调整卡片顺序**
   （悬停卡片上方出现插入指示条，松手即插入该位置）。
8. 完成后接续：详情「自动执行」→ 模式切到「完成后接续」，留空次数 = 无限——先点
   「执行」手动启动一次（任务此前从未手动启动时，接续不会自动开跑），此后每次完成后
   自动续跑，卡片持续「进行中」；设次数上限则跑满后停止并落「已完成」。
9. 刷新页面/重启 DSH → 任务仍在；卸载插件 → GUI 恢复原状。
