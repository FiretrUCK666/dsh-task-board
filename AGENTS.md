...
- **共用部件**：`ui.tsx`（Button primary/ghost/danger + `size="sm"` 紧凑变体 + `pressed` 按压态、Section、
...
- `tasks.ts`（任务状态机/plainRunsOf/COLUMNS/latestExecutionOf/chainUnlimited/taskColumnAllowsAutomation）、`schedule.ts`（cron）、`scheduler.ts`（每分钟 tick + cruiseTick）、`cruise.ts`（巡航窗口）、`session-activity.ts`（原生侧对账）、`colors.ts`（卡片色板 PALETTE + withTaskColor，标签已删仅留颜色）、`session-list.ts`（统一会话列表：taskSessionsOf 去重/优先级/隐藏 + sessionWindowOf 窗口派生）、`store.ts`（ledger 持久化）、`execution.ts`（投递与结算）、`controller.ts`（台账+统一并发调度器）、`question-rpc.ts`（原生问答 wire 模型）、`automation.ts`（会话规则 + automationRowsOf 单一投影 + sessionRuleReadiness/sessionRuleOf）。
...
- **会话状态派生/未读**：`session-display.ts`（waiting>running>settled；`viewedAt` 基线）；状态 chip 的**唯一派生** = `session-chip.ts`（`sessionStateChip`：等待/运行=warn+spinner，settled 标签由调用方给——执行行命名结果、链接行命名活动，另 `waitingKeyOf` 消除一切 `as 'waiting.approval'` 强转），TaskDetail 执行/链接两分支、ReviewDetail 状态行、SessionDetail 状态行全部走它；共享 transcript tail（3s 水位轮询/贴底/上翻暂停）；统一会话行 = `SessionRow` 单骨架 + TaskDetail 的 `SessionActionRow` 单构建器（执行/链接已合一：同一行文法——**行首同类图标（运行=play、绑定=link，同槽位同尺寸）+ 会话标题 + 工作区标签（仅绑定行：它是绑定身份的一部分；运行行会话属于任务自己的工作区，运行配置已体现，绝不显示猜测默认值）** + **恒有状态 chip（链接行空闲=cancelled 态显示「空闲」，任何行不再「一个有、一个没有」）** + 操作；meta 行 = sessionTimes 或 sessionWindowOf 的 开始/结束/耗时、无窗口则「更新于」；footer = CommentSummary 评论摘要（无正文只留 评论N+时间+chip）、动态/错误留槽）。
...
- `tests/session-activity.spec.ts`：原生侧活动对账纯逻辑（基线不补记、翻转检测、open round
  与直发抑制、refine 标记、grace 常量）与 `latestUserMessageText`（外源轮文本捕获）。
...
- `tests/controller.spec.ts`：端到端含**绑定瞬间同步**（拖入 running→立即进行中/补外源轮/
  ...
  + updateSessionRule 原地编辑 + 非驱动列 paused 保位不触发** 与**复制为模板置顶**。
