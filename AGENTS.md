...
- **浮层几何（看板盒参照，全宽度统一）**：`.modalBackdrop` = 看板盒内 absolute inset:0 + 只纵滚（`overflow-y:auto` + 隐条——舞台不出条，可见条只属于面板唯一滚动体）+ `margin:auto` 子项居中（**不用 align-items:center**——超高面板顶部会被裁到滚动之外），底部内边距携 `--dsh-tb-kb`（键盘内缩单一变量，见 keyboard-inset.ts——浮层随剩余空间收身，标题/动作永不被键盘顶出屏幕）。**浮层家族 chrome（`.modal/.detail/.review`）共用一条声明**（margin:auto/不透明浮面/边框/radius-xl/shadow-3/入场动画 `--dsh-tb-motion`），各面板只加自身 width；**宽高一律板盒 % 参照，禁 vw/vh**（确需视口参照才用 `dvh`）。**`Dialog` 默认 portal 到看板盒**（不 portal 反成需显式 `portal={false}` 的例外——浮层 backdrop 锚最近的 positioned 祖先会被其 overflow:hidden 裁剪，NewTaskModal 曾因漏传被板头压死「运行配置全失效」且时好时坏=板头高度变化；`SessionFrame` 恒 portal）；**`Dialog` 一处注册 Escape 关闭，全家族获得**。窄屏（`useNarrow()`）下锚定弹层换形态：巡航设置用 `Dialog`（弹层几何在小板盒上不可靠），宽屏保持 popover。
...
- **动效唯一语法**：注意力 = `--dsh-tb-attention` + `--dsh-tb-breath`；卡片外层呼吸环（`.card[data-unviewed]`/`.card[data-active]`）、会话行内层柔晕（`data-glow` 绑定、pause 而非 cancelled——永不闪烁）、浮层入场 `dshTbDialogIn`。**`prefers-reduced-motion` 分两类处理（不是全部静态降级）**：**装饰动效**（入场/hover/FLIP/smooth scroll）静态降级；**功能性状态指示器豁免**——`.spinner` 保留旋转（只放慢到 1.4s，静止的转圈=「卡死」假象、与"运行中"真相相反，手机开系统减少动画也必须能看到进度），呼吸环降级为静态环（状态仍由颜色/环传达）。判据：动效本身即信息者存活，只为悦目者舍弃。
...
- **拖拽与落位**：插入条 = `drop-position.ts`（`insertionGapOf`/`indicatorTopOf` 纯函数）+ 行/盒内 22px 底部留白（否则尾部槽被裁剪）；**拖拽自动滚动** = `drag-autoscroll.ts`（滚动期间根元素 `scroll-behavior` 临时置 auto——smooth 与逐帧滚动打架）；**FLIP 落位 = 结构驱动**（`use-flip.ts` 比对列+索引的 DOM 结构，滚动/hover 永不触发；本地拖拽、详情按钮换列、远端 applyRemote 三种驱动共用同一条动画）。**横向滑轨上的移动必须可见**：命中跨列变化先把目标列瞬时滚入视野（smooth 会与被测 rect 打架）再播动画。会话排序**只有按住拖拽**一条路径（不再提供按钮对）。
...
- **响应式与触屏（改窄屏/触摸问题先读这里）**：① **单一响应式参照 = 板盒宽，不用视口**——`[data-dsh-taskboard-view]` 设 `container-type: inline-size` + `container-name: dsh-tb`（并带 `-webkit-text-size-adjust:100%` 抗安卓/微信 font boosting），几何断点全部写进 `@container dsh-tb (max-width: …)`（compact 档 680px、review 堆叠 600px=双列真实下包络：弹性 rail `min(344px,46cqw)` + 转写最小可读 + 内缩）；侧栏折叠/分屏/手机都自动正确，**板体布局永不用 `@media (max-width)`**。② **compact 列 = 自由横向滑轨**：`flex: 0 0 clamp(200px, 46cqw, 320px)`（约两列同屏、邻列露边暗示可滑）+ **列导航 tab = 五等分具名 grid**（`.columnTabs` 基显 none、compact 显现 `repeat(5,minmax(0,1fr))`——内容宽胶囊必溢出、导航条自己还要滑等于没有导航；名称 `.columnTabLabel` ellipsis、点与计数 `flex:none`，列头文法原样复用）；**轨道不用 scroll-snap**；**列身份是唯一真相、px 永远派生**：`activeColumnIndexAt`（位置→列）与其逆运算 `scrollLeftForColumn`（列→位置，钳进可滚范围）两个纯函数承担全部换算，tab 直达与 `.columns` 上的 ResizeObserver 宽度重锚共用后者——容器宽度一变就把 scrollLeft 重新瞄准同一列左缘，**开合侧栏/旋转/分屏不再每次"往右偏一点"**（snap 时代的漂移病，只删 snap 是减半、锚定才是灭绝）；compact 底 `env(safe-area-inset-bottom)`。③ **触屏 = 桌面 parity**：交互一件不改（悬停显露保持悬停显露、拖拽保持拖拽、不添加任何按钮副路径），`@media (hover: none) and (pointer: coarse)` 块**只承载隐形人体工学**——小控件 `::before` 透明热区扩到 ≥36px（视觉 28/24 节奏不破）、色点 swatch 24px、输入 `font-size:16px`（防聚焦自动缩放）。④ 行为/结构开关只认 `useNarrow()`（matchMedia 单一信号，用于弹层换形态这类 CSS 无法表达的分支）。⑤ **会话行 = 具名 grid**（`.sessionRowTop` 槽位 lead/chip/act，compact 重排两行 `"lead act"/"chip act"` 且标题槽硬 `minmax(60%,1fr)` 配额）——chip 与动作在每行同一 x、永不随标题长度换行漂移（内容驱动 flex-wrap 是"排版乱、没对齐"根因）；**行动作 = 标签/图形一对**（`.rowActionText` + `.rowActionIcon`，宽档显字、窄档折字显图形，tooltip 恒留文字）：窄档动作簇不再是 ~156px 固定税（"标题只剩三五个字"根因）。⑥ **滚动跟随 = 一条机制、无模式分支**（`use-transcript.tsx`）：`resolveScroller` 从内容区向上找**真正在滚的元素**，`useFollowScroll`（解析根 + window 捕获阶段 scroll 监听〔scroll 不冒泡但走捕获〕+ 在底跟随 + jumpToBottom + ResizeObserver 同盯内容与解析根）被转写尾与评论线程共用——桌面滚内层区、手机堆叠滚面板 body，同一份代码自动正确（把"内层区就是滚动根"写死，曾让手机端跟底失效、"滑到最新"永不出现、必须一路从顶拉到底）。⑦ **滚动条文法**：可见条只属于贴表面边的滚动体，且**横向内衬由滚动体自持**（`.interactionCard` 只留纵向 padding，卡内滚动体与钉住的动作行各自 `padding: 0 12px`）——否则 bar 浮在边内 12px、紧贴文字。⑧ **堆叠档评论界面 = rail 优先**（`@container dsh-tb (max-width: 600px)`）：`.reviewRail` `order:1`（状态行/可折叠上下文与配置/评论/composer 直达首屏）、`.reviewMain` `order:2` 且 `max-height:45cqh` 独立滚——几十条转写不再把发送框推到屏幕之外；`SessionRail` 的 rail 头带折叠行（复用 disclosure 文法，默认展开）。`tests/mobile-contract.spec.ts` + `review-page.spec.ts` 钉死以上全部。
...
- **运行态与会话集合唯一推导** = `task-live.ts` + `linked-sessions.ts`：`relatedSessionIdsOf(task, linkedIds)`（refine → 显式 session 绑定 → 执行轮 → 注入 linked，去重稳定序；一切相关面只经此推导）；`taskLiveStateOf`（waiting > running > idle，running 以相关会话原生 `byId.running` 为真相）。`deriveLinkedSessions`：**链接行只来自显式 session 绑定**——工作区绑定是来源/配置关联，**永不派生成员会话**（主界面新建的会话绝不会自己进卡片；此为有意的产品语义，勿"恢复"）。**原生归档即时同步**：`taskSessionsOf` ctx `archivedOf` 是唯一过滤点（读 `workspaces.list` 归档集），归档会话的行（执行行+链接行）整体离卡、`linkedOf` 同源收缩（相关集/计数/liveState 同帧一致），台账轮次保留、取消归档带历史回来；行派生自原生真相非台账→两端零延迟（workspaces.list 订阅 notify 即重推导）。
- **原生活动检测** = `session-activity.ts`：外源轮双通道 + **每运行期恰一轮**——主通道 mux `user/message` 帧（`nativeTurnOf` 解析，直达 `controller.recordNativeTurn`）；兜底为**状态式**扫描（相关会话当下在跑且本期未消费即记轮，覆盖页面加载已在跑/出生即在跑/断流窗口；`book.recorded` 按运行期消费、会话回落 idle 即重新武装），两通道经**轮次锚点**（`ExecutionRecord.anchor` = 启动该轮的 user 消息 seq）跨通道/跨设备/跨引擎交接幂等去重；正文 = `latestUserMessage`（最新 user 消息即真相；纯图片 → `imageOnly` 占位）。
- **同步域（多端一致的地基）**：`board-doc.ts`（**BoardDoc + 合并文法唯一居所**：逐记录以**作者声明**裁决——commit 携 `changed`（本副本相对上次同步基线内容移动过的 id 集，由 `changedIdsOf` 对当时视图 diff），claimed 记录无条件接收（host 串行到达序 = 后到者胜，**设备时钟不参与裁决**；内容相等的 claim 是 no-op），未 claimed 维持 LWW（陈旧副本绝不覆盖新编辑），删除带 `baseUpdatedAt` + 墓碑（at=所见最新+1，抗偏移压复活、真更新可复活），`stamps` 记 host 接收钟；**读态不是作者**——`viewedAt`（任务级与轮级）在 claim 比对前被剔除（`authorshipKey`），"点开卡片"永远不产生声明，否则陈旧副本会凭一次点击无条件覆盖引擎刚记的外源轮（轮消失、列回退、`recorded` 已消费不再补记 = "一点卡片就不动了"的复发机器）；读态改走**单调 max 合并**（`mergeReadState`，按任务与轮 id）——看过没有只前进不回退、永不参与内容裁决；section 层同构 claim——commit 携 `sectionClaims`（本副本 dirty 的 section），claimed section 无条件接收 + host 时钟落 at、未 claim 的基线副本一律跳过（陈旧 at 绝不再覆盖），无 `sectionClaims` 字段的旧客户端回落纯 LWW；`normalizeBoardDoc`/`normalizeCruiseValue` 把介质的值当数据不当真相）· `host-sync.ts`（`BoardSyncClient`：boot 一次性迁移〔host 空 → 整视图 bootstrap；host 非空且本地分歧 → 按记录联合并入、共享 section 归首写者、分歧整视图 `onBackup` 停放〕、claims 在 `setTasks` 逐次累积 / 全量 ack 后清空（陈旧全数组快照永不误 claim 回退远端编辑）、去抖提交（在途合并 / trailing refire / 失败退避）、SSE + 轮询 + 重连汇入单一 resync、引擎租约心跳（任何 API 命中即续期，携带 `active`=本 tab 可见性，**转后台即刻写 `active:false`**）、`requestLaunch` 中继；`hostProtoVersion()` 透出 host 的租约协议版本〔无版本=旧 host〕；`Synced*Store` = 既有 store 接缝 over 共享文档 + 本地镜像）。**用户意图写必经 `controller.userEdit(taskId, mutate)` 单一漏斗**（返回新对象即结构上强制 bump `updatedAt`——隐藏/删除/规则开关/排序等 8 处"静默吞写"根因灭绝；引擎派生写与读态写 viewedAt 不走此漏斗，各有自己的新鲜度语义；排序被挤位的 sibling 也补 stamp）。

### 关键不变量（避免重造已有机制；改前先读对应文件）

- **多端同步（host 唯一真相 + 引擎租约，一切数据一致性问题的根解）**：看板真相 = host 的 `BoardDoc`（storage hub json 后端持久化），localStorage 五键**降级为离线镜像/草稿/备份**（键名不变）。浏览器 = 乐观副本：本地写即时生效，经 `store.save → Synced*Store → BoardSyncClient` 去抖提交，**host `applyCommit` 合并后返回权威文档，所有副本向它收敛（无 CAS 也无不收敛）**；远端变化经 SSE（+ 轮询兜底 + 重连/回前台对账）→ `controller.applyRemote`（**永不回写**——回写即回声）。裁决文法见「同步域」：**作者声明（changed）优先、LWW 兜底、读态走单调合并、墓碑压复活**——设备时钟好坏不影响"谁的最近操作生效"（会话拖拽排序等任意字段两端必达）。「添加已有会话」与侧栏拖入同走 `addTaskSource`。**引擎租约**：host 内存租约（TTL + 任何 API 命中续期 + SSE 断流宽限期提前过期），**只有引擎端**跑 dispatch 泵/scheduler tick/reconcile/外源轮记轮/cruise 翻转/settledFollowUp+链续；非引擎端 = 纯视图 + 提交器，用户动作照常写台账（同步后由引擎泵出），手动执行经 `requestLaunch` → host `/command` → 引擎 SSE 执行（无引擎时 park，下次授约重放）——单泵 = 单并发预算 = 多端绝不双发（含同浏览器双标签）。**席位跟可见性（双向即刻）**：`/lease` 携带 `active`（本 tab `visibilityState`），**转后台立刻写 `active:false`**（不等心跳——冻结前最后一笔若是 active:true，抢夺条件永不成立），**可见端可从不活跃持有者抢夺席位**（SSE 广播旧持有者即时降级、新引擎上任即 reconcile 补记漏轮；双可见端不互抢、全后台维持最后持有者让定时照跑），回前台立即续租+resync——引擎永远坐在用户正看着的那台设备上（"手机点开始没反应、电脑端才动、刷新才好"根因）。**流活性有界**：SSE 计数只在距持有者上次真实 HTTP touch `STREAM_ALIVE_MAX_MS` 内续命（半开 socket 不等于活着；否则冻结设备的席位永不可抢、全 fleet 饿死）。**协议版本 `LEASE_PROTOCOL_ACTIVE`** 随 LeaseState 下发，client 记 `hostProtoVersion()`——**旧 host（无版本）= 抢夺不生效**，板头据此如实提示"服务端待重启"（host 半区改动须重启 dsh web 才生效这件事，从"用户猜"变成界面可见）。**板头引擎指示**：同步模式下本机非引擎且有排队/待注入 → 安静提示"引擎在另一端"；空闲永不唠叨。**投递看门狗**：open 轮超 `OPEN_ROUND_WATCHDOG_MS` 且其会话 idle 无 turn 证据 → 合成 cancelled 结算释放（会话在跑/等人回应绝不判）；reconcile 也扫**停车卡**的 open 轮，`resolveCardDrop` 拒绝带 open 轮离列——僵尸轮曾会占住并发预算并吞掉该会话一切后续外源轮（"行亮着、卡片永远不动"的永久机器，一处灭绝）。question tracker 每端独立流（先到先答，官方语义）；drafts 刻意设备本地不同步。storage hub 缺席 → 整体回退 localStorage 模式 = 旧行为，功能不降级为错误。
- **统一会话与评论单轨**：相同会话 = 同一条线程（`sessionCommentsOf`）；直发/驱动/评论并轨为一种留言；发送两态 = 排队（调度器/巡航/FIFO，可取消）与插话（`steerComment` 立即送达）；**图片只有一条官方链路**：`sessions.prompt` 的合法 image part 是 `{type:'image', mediaType, data(base64), name?}`——契约即政策"浏览器送临时字节、**host 做持久 admission**"，故 composer 的 `DraftImage` 经 `toPromptImage` 直发（`PromptImage` 贯穿 sessionMessage/steerCommentWithImages/sendRawMessage/sendComment），**绝不自建附件桥/上传路由**（自造 `{type:'image', attachment:{…}}` 不在合法 union 内、host 必拒 = 双端图片全废的根因，勿再重演）；渲染侧 admission 后的存储形状是 `{type:'image', attachment: ImageAttachmentRef}`——折叠 `contentImagesOf` 提取（去重、坏 part 跳过、纯图消息不丢），`MessageImage` 经官方 `sessions.attachment` 读回 base64 渲染缩略图（复用 `createTranscriptReader`：图片不可变→长 TTL、在途合并、超时）；客户端白名单/20MB 上限与就近 `.formError` 透出保持，附件条独立成行、动作行恒单行（发送按钮永不被 chips 挤没）。
- **插话与任务运行态（单一推导，杜绝各面各判）**：发送层 mode 参数化——`sessionMessage`/`sendRawMessage`/`commentRun`/driver.prompt 都收官方 `'queue' | 'steer'`（官方 prompt 枚举）；**三入口统一**（评论线程插话 → `steerComment`('steer')；会话规则 `send:'steer'` → 建轮**即带 injectedAt**（dispatch 按注入标记过滤，杜绝「同一轮双注入」竞态）+ 立即注入（结算/续跑机制与排队轮完全相同）；直接留言/交互卡回答不走此层）。**运行态与相关会话集唯一推导 = `task-live.ts`**（见核心层）：插话/规则/外源/执行/显式绑定一视同仁；`sessionDisplay` 以 `nativeRunning` 参数把「settled 轮但会话真在跑」显示为 running（waiting 仍最优先）；`reconcile` 的 `driveLiveStates` 驱动**直发轮**（settle-at-birth 无事件路径）——会话跑 → 卡片进入进行中，会话停 → 落「待审核」并走**同一条 `settledFollowUp`**（插话完成也是完成）；板内各轮的事件结算路径不变、落列归它们（`isDirectLike` 只认已结算直发轮，永不双结算）。**一致性铁律**：行徽章（liveState）与卡片列（台账轮次）必须同涨同落——只要相关会话 running 且该轮未记，双通道保证记轮并把卡移入进行中（这正是"行显示进行中、卡片不动"曾被打破的地方）。
- **官方 @ 引用机制（唯一桥 = reference-source.ts）**：所有输入框共用同一桥——与主界面 ui-reference 相同的两个 Remote 命名空间（`remote.fileReferences` + `remote.sessionReferenceResolver`，结构化读取，缺面降级为无 @ 菜单）。**官方身份政策（不可绕过、勿在代码里绕，参见 README 能力边界）**：目标会话为子代理路由会话（`origin === 'subagent'` 或挂在活跃父代理下）时宿主对一切通用 RPC 返回 `agent-busy`，官方 composer 同败（官方语义，无修复版本）。**插入全用官方文法**：`file-reference-grammar.ts` 与 `session-mention.ts` 为官方包**逐字镜像**（打包门禁禁跨插件值导入，镜像 + 契约测试钉死）；会话引用 = `@[label](dsh-session:…)`，解析全走官方链路——板子只产出官方文本。作用域：`PromptInput` 以目标 `sessionId` 唯一解析（`referenceSessionOf`：refine→执行→绑定→当前会话→列表首项）；`@"` 引号路径内不弹会话候选（官方规则）。**失败契约**：`listReferenceRows` 永不 reject（两域独立降级、坏行跳过 `diag.skipped`、`createdAt` 非法只省略日期段）；官方 log-only——菜单**绝无失败文案**，唯一例外 = 会话域官方失败时**降级为板内目录行**（`referenceSessionCatalog` + `catalogSessionRowsOf`：排自身、上限 50、同样式、插入仍是官方 mention；只降级"发现"，解析 100% 官方）；两域皆成功且真无候选才显示 `prompt.noReferences`；效果链带兜底 catch，绝不留空菜单死等。**菜单三文法（纯函数）**：能力门 `referenceMenuAvailable`；空态文案专属；`continueAfterPick` 只在目录下钻（`@"路径/`）重开菜单（插在活 token 上盲重开是"无匹配命令"回归根因）。
- **原生交互卡 + 上下文块**：agent 挂起时 `InteractionCard` 实时弹出，**回答只走原生 mux 通道**（`connection.api.respond`），普通留言不解决挂起提问。to-do 读**官方 `todos` projection**（与原生 TodoPanel 同源、宿主升级自动跟随；投影缺席降级事件解析）；goal/子代理走 session-state 桥（缺面即降级）。`SessionContextBlock` 是 rail 内**浮层**（默认折叠一行，展开独立滚动、外点关闭），**有未完成的才显示**：`contextWorthOf` 唯一判定——todo 任一未完成 / goal 活跃 / 子代理未完成（status 缺失保守视为进行中）；全完成才整块隐藏（完成 todo 留勾+划线）。**行文法**：标记 + 文本两元素——标记 `flex: none` 贴首行（`align-items: flex-start`），文字进 `.chipBody`（裸 chip + nowrap 会溢出盖正文）。**桥读原生形状**（session-state-route，`listChildren` 按父会话 id）：goal = `{objective, phase}`（phase complete 即不返回）；子代理 = `{label, activity}`（只返回在跑的 child；inactive = 已结束）；旧实现的 `activeGoal`/`title/status` 错误读法**永不回退**。
- **原生侧同步（外源轮）**：机制详见核心层「原生活动检测」——**事件主通道 + 状态兜底 + 锚点幂等，每运行期恰一轮**；外源轮不排队/不注入、驱动卡片「进行中」、跑完经普通结算落「待审核」（`EXTERNAL_SETTLE_GRACE_MS` 内无回合证据 = 噪声取消，不假停留）。主动添加（拖入源/新建/选入）经 `reconcileBoundTask` **即时同步当前实况**（当下在跑即记轮 + 未读呼吸；空闲不虚构）；一切写入在 await 之后以**当前记录**重建并复查守卫——陈旧快照绝不覆盖新账；已完成的过去回合永不回补（不在跑即非候选）。
- **多源绑定（拖入 = 添加，绝不刷新替代）**：`TaskRecord.binds: TaskBind[]` 是真相源；所有读者走 `taskBindsOf(task)` 投影；`copyTask` 不复制 binds 与会话规则（模板语义）。**session 绑定 = 会话出现在卡片上**；**workspace 绑定 = 仅来源/配置关联，永不派生会话行**（见核心层 linked-sessions——主界面新建会话不再自动进卡片是产品语义，非回归）。**隐藏托盘「删除」= 从任务移除**（`removedSessions`，其轮次同删）；**把该会话再拖回可恢复**——`addTaskSource` 清 removed；工作区重新绑定不恢复任何会话（成员本就不在卡片上）。**`removedSessions` 是权威「非相关」门**：`taskSessionsOf`/`linkedOf`（显示）与 `relatedSessionIdsOf`（task-live 运行态/外源轮/rename 守卫/添加候选）一律减去它——被删会话既不进卡片行、也不驱动 liveState/外源轮（workspace bind 在场时其 session bind 会残留，减 removed 才封死「删了还在原生聊天→卡片进进行中」），并重新成为「添加会话」候选。
...
- **自动化两半互不干扰**：任务级 `schedule`（按时间表/完成后接续）与会话级 `rules`（指令 = 自定义或任务执行 Prompt（usePrompt，发送时取当前值非快照）；触发 = cron 或每次完成 on-complete）字段正交、语义独立（关任务级不影响会话级——controller 单测钉死）。UI 唯一 = `AutomationEditor`（板与详情同一份；**启用开关只在编辑器内**；总览卡片 = 身份头 + 一行摘要 + 展开体）；分节用 `Segmented` 分段文法。**on-complete = 永续循环**：**任何一次完成结算**（普通执行/用户评论轮/外源轮；完善轮与规则自己的轮除外）都触发，不受列暂停约束；**所有结算路径共用同一条 `settledFollowUp`**（live watch 与恢复/对账都走它——冷会话/刷新/漏掉的翻转结算成的"完成"同样触发，杜绝"完成却无反应"）。规则轮走**自动化车道**（ruleId；`nextEligible` 优先、**不受巡航开关约束、绝不等巡航**；全局 FIFO + 预算；每规则在途至多一轮）；规则自己的轮**成功**结算即续下一轮，直到关闭/删除/会话消失/内容不可用；失败/取消不续；用户手写评论（无 ruleId）永不触发。**显示真相**：已启用未触发 = 「等待任务完成触发」chip；排队/插话只是模式标签绝不误读为进行中；**每会话至多一条规则**；老数据归一化为 cron + 自定义指令。
- **完成接续（链）语义**：武装即开跑（`setSchedule`：`enabled && chain && 可执行 && 无在途轮`，任意列，不等手动重执行）；每次**普通执行成功结算**瞬间接续下一轮（失败/取消/完善轮不续；预算到顶自动解除）；**手动移动卡片（除 done）绝不解除武装**（链只在结算瞬间动作）；`done` = 硬停。`ruleReadiness` 链模式跳过列暂停；调度器链恢复 tick 以「最新一轮已结算且未到预算」为条件。
- **巡航窗口 v4 文法**：**总开关主权**——`enabled` 是当前状态、用户点为主；窗口是计划表，只有「开始/结束时刻」越过时翻转开关；**增删窗口绝不动开关**（唯一例外：新增「只填结束」窗口，其开始时刻 = 创建时刻）。窗口 `{startAt?; endAt?}` 三态（都填=区间 / 只填开始=到点开并保持 / 只填结束=创建即开到点关）；列表**自动排序**（`sortWindows`：立即开启→按开始→同开始按结束，开口最后）；每窗口一行文法（`cruiseWindowLabelOf`——「开始 → 次日 结束」/「{time} 起保持开启」/「已开启 · 至 {time}」，完整时刻走 tooltip）；**「次日」只用于紧邻后一日**（`isNextDay` 按日历日，跨月年正确），间隔多天按真实日期；**状态行永远一句话解释开关为何是当前值**（`cruiseStatusLineOf`）。校验 = `windowRangeIssueOf`（归一化后判定，各形态专属内联提示，跨午夜只支持一晚）；`normalizeWindow` 对非法范围绝不虚构次日。
...
- **运行配置预设（自定义 + 默认应用）**：`src/core/run-presets.ts`（键 `dsh.taskBoard.runPresets.v1`）；内置不可删 `deploy-default`（空配置 = 部署原生默认）；**默认回退链** = `defaultRunPresetOf`（defaultId 缺失/悬挂/被删 → 一律「部署默认」）；UI 唯一 = **`RunConfigFields`**（配置预设 picker + RunConfigEditor 整块组件，TaskForm 与 NewSessionModal 渲染同一份——零漂移）+ `RunPresetManager`（添加/编辑 = 名称 + RunConfigEditor 同字段自编辑，新建以当前表单配置为起点、删除、设为默认），新建任务 `initialDraft` 应用默认预设（草稿优先）。
- **会话标题（与原生一轨，绝不打架）**：原生 rename = 官方「钉住」语义（`sessions.rename` 以 `user` 源追加 `session/title`——钉住标题、作废在途自动生成、后续消息永不自动改名；不命名则宿主自动命名链完整保留：首条消息 → 确定性 fallback + provider cadence）。板内两条入口都走同一官方写：**新建会话的标题选填**（`createTaskSession` 的 `title`——填了走官方 rename 钉住，留空什么都不发、交给宿主自动命名，绝不制造占位名与原生打架；失败 = `titleError` 部分成功）；**会话行重命名**（`SessionRow` 行内铅笔 → `renameTaskSession`，目标必须是本任务相关会话，空标题拒绝，成功只 notify——标题真相已在宿主 list，无第二份状态）。**未命名文法**：宿主未命名的会话行统一显示 `detail.sessionUntitled`（`sessionRowTitleOf` 唯一推导 + `untitledLabel` 注入，run 行与 id 命名的 linked 行都覆盖——绝不回退任务标题或裸 id）。**认得宿主的确定性自动名**：判据唯一居所 = `linked-sessions.realTitleOf(title, cwd)`——durable `title` 恰等于会话 `cwd` 的 project basename（`workspaceLabelOf`，含 Windows/POSIX 分隔与尾斜杠）即宿主首条消息后的 fallback 名、非真名，返回 undefined 走未命名文法，直到 provider 生成名或用户 rename 钉住。**两个读者共用这一处**：`controller.sessionTitle`（执行行/面板头）与 `rowOf`（链接行）——曾只修前者，链接行仍拿 `source.title ?? workspaceLabel` 把文件夹名当标题（"不填标题却显示工作区名"多次未修好的真正根因）；`rowOf` 的标题槽**只放真名**，无名保持 id 交给未命名文法，工作区名只属于它自己的 `workspaceLabel` 槽（用户故意把工作区名当标题为可接受的唯一歧义边界）。
- **详情页新建会话（会话区「+ 新建会话」）**：入口住在会话区动作行（`.sessionToolbar`——hint 左、按钮右，紧贴列表上方，空列表常驻；不放区块标题旁）；`NewSessionModal` = `Dialog portal`（嵌套弹层硬性规范）+ `.modalScroll` 唯一滚动体，正文 = 选填标题 + 运行配置（初始值 = 任务自身配置）。链路 = `controller.createTaskSession` → `ExecutionService.createSession`（**保证全新会话**：client `sessions.create` face 结构化读取 + wire `sessions.create` 兜底；face 缺失降级 connectWorkspace blank 复用）→ 按执行同序应用配置（model → agentPreset → `/permission`）→ 标题（官方 rename，见上条）→ `addTaskSource` 绑定（多源叠加/持久化/即时 reconcile 全复用）。**失败契约**：创建失败 = ok:false、任务不动；配置/标题失败（会话已在）= 诚实部分成功 `configError`/`titleError`，会话保留、弹层内联展示——绝不静默吞掉，也绝不回滚已创建的会话。新会话不是执行：无执行轮、不进调度器、不涉自动化。
- **交互卡/rail 永不横向爆框、纵向不挤**：rail 的 flex 子项全部 `min-width: 0`；行内按钮组可换行（`.interactionActions` flex-wrap）；卡片 `margin: 0 14px`。**高度契约**：`.interactionCard` `max-height: min(320px, 40vh)` + `overflow: hidden`，正文在 `.interactionCardBody`（`overflow-y: auto; min-height: 0`）内滚动，**动作行钉在卡外永远可见**（长计划挤压 bug 根因）。**内衬归属契约**：卡只留纵向 padding，横向 12px 由滚动体与钉住的动作行各自持有——滚动条因此落在卡内缘（贴边），不会浮在边内紧贴文字（与转写区同一条"滚动体自持内衬"文法）。`.reviewRail` `overflow: hidden`；`SessionRail` 交互卡位于 `.sessionRailScroll`（评论 + 卡同一滚动区，贴底自动跟随），composer 钉在 rail 底部；rail 头（上下文计量 + 运行配置）带折叠行，窄堆叠档一键直达评论。合同由 `review-page.spec.ts` + `mobile-contract.spec.ts` CSS 契约钉死。
...
- **草稿覆盖矩阵（`drafts.ts`，键 `dsh.taskBoard.drafts.v1`，设备本地不同步）**：承诺 = "任何会随切换卸载、且承载用户手写内容的输入面，未提交内容必须回来；提交/保存/明确取消即清槽"。现行键：`comment:<taskId>:<sessionId>`（评论 composer，评审页与链接面板共用同一身份）、`edit:<taskId>`（任务编辑表单，整份 TaskDraft JSON）、`refine:<taskId>`（完善答案）、`new`（新建任务弹窗）、`newsession:<taskId>`（新建会话标题 + 运行配置 JSON；损坏回落任务自身配置——草稿是便利从不是真相）、`rule:<taskId>:new`（会话规则 ADD 模式的指令；**编辑既有规则永不用草稿**，保存的指令才是真相）。瞬时单字段输入（搜索框、时间字段、选择器、巡航窗口）刻意不入矩阵：没有"写一半"的内容可丢。新增输入面按同一矩阵归位，不另起存储。
