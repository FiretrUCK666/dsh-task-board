...
- **响应式与触屏（改窄屏/触摸问题先读这里）**：① **响应式参照 = 表面自身的宽度，不用视口**——`[data-dsh-taskboard-view]` 设 `container-type: inline-size` + `container-name: dsh-tb`（带 `-webkit-text-size-adjust:100%` 抗 font boosting），板体几何断点全写进 `@container dsh-tb (max-width: …)`（compact 680px）；**评审家族锚定面板自己**——`.review` 声明 `container-name: dsh-tb-panel`，头部/堆叠/列宽全部 `@container dsh-tb-panel (max-width: 600px)`（面板是唯一诚实参照：板宽而面板被 880px 封顶时头部照样该换行；面板不能查询自身，它的宽度内衬规则留在板容器 648px 档）；**板体布局永不用 `@media (max-width)`**。② **compact 列 = 自由横向滑轨** `flex: 0 0 clamp(200px, 46cqw, 320px)` + **列导航 tab 五等分具名 grid**（`.columnTabs` 基显 none、compact `repeat(5,minmax(0,1fr))`）；五等分只剩约一个半汉字的标签预算，故 tab 消费 `STATUS_SHORT_KEY`（`board.statusShort.*`，与全称同表的 locale 数据）、完整名进 `aria-label`、色点窄档让位（列头已有同色点）；**不用 scroll-snap**；**列身份是唯一真相、px 永远派生**：`activeColumnIndexAt`（位置→列）与逆运算 `scrollLeftForColumn`（列→位置，钳进可滚范围）两个纯函数承担全部换算，tab 直达与 `.columns` 的 ResizeObserver 宽度重锚共用后者（开合侧栏/旋转/分屏不再"往右偏一点"）；compact 底 `env(safe-area-inset-bottom)`。**板头 = 两行确定的成员归属，不是折行汤**：成员归组（`.boardModes` = 整理+自动化、`.boardState` = 运行/排队+引擎指示，状态零段省略——「排队 0」是噪音）；**导航行 = 返回 + 板名 + 状态组（左）… 空位 + 自动巡航 + 新建任务（右端）**——状态是"看"的放左边、巡航与新建是"做"的放右边、一次性动作永远在最右；**工具行 = 筛选（左，与返回/板名同一条 x）… 空位 + 整理 + 自动化（右）**——筛选与它过滤的列表同侧，模式按钮成簇在右。compact 各自确定性地换行（不是内容驱动的折行），且**换行由具名网格区域说**：导航行 = `"back title new" / "state state cruise"`（第 1 行 返回+板名+新建任务，第 2 行 状态跨两轨 + 巡航守右），工具行 = `"search" / "modes"`（筛选独占整幅白长条、占位符永远完整；模式组独占一行右对齐）。**为什么巡航不跟新建任务挤在第 1 行**：320px 手机上第 1 行的固定成员已是 返回28+巡航132+新建96+间隙24=280，板名只剩 16px——「标签一律保留」被算术当场否决，所以巡航回到状态行（桌面仍是巡航在左、新建在右，用户要的是桌面那条）。**宽档推右靠唯一 `.boardSpacer`（flex:1），绝不用 `margin-left:auto`**（auto margin 只右对齐"所在行的首项"，一换行即散架）；**标签一律保留**（藏标签换宽度是错误取舍）。③ **触屏 = 桌面 parity**：交互一件不改，`@media (hover: none) and (pointer: coarse)` 块**只承载隐形人体工学**（`::before` 热区 ≥36px、色点 24px、输入 16px 防缩放）。④ 行为/结构开关只认 `useNarrow()`。⑤ **会话行 = 具名 grid，可选成员绝不靠自动落位**（`.sessionRowTop` = lead|chip|act 三槽，端线同文法——chip 与动作同一列 x；compact 单行同构：动作收图形(`.rowActionText`→无、`.rowActionIcon`→显)后 lead 拿 `minmax(0,1fr)` 整余轨，**`.sessionRowLead` 是单轨具名网格 `"identity"`、身份块显式 `grid-area: identity`**——曾经的 `12px minmax(0,1fr)` + 自动落位，在链接行（从不渲染未读点）时把整块身份推进了那个固定 12px 轨 → 名字轨算出 0px → 标题只剩一个字、工作区胶囊消失：**「可选成员占固定轨」是结构性 bug，不是样式问题**；未读点改为**覆盖在 12px 图形轨右上角的徽标**（`position:absolute`，偏移由 `--dsh-tb-lead-icon`/`--dsh-tb-row-line`/`--dsh-tb-dot` 派生），因此有没有徽标每行左边界恒等；identity 网格 = icon+标题 line1 / **工作区跨双轨 line2（自行内容边起）**，状态 chip `padding-left:0`——**图形、工作区、chip、meta 全部同一条 x**（用户要的"和标题左边那个符号对齐"），行内 4px 节奏；动作键 `align-self:start` 对标题线。⑥ **滚动跟随 = 一条机制**（`use-transcript.tsx`）：`resolveScroller` 按**「谁被声明为滚动体」**（自身或最近祖先 computed `overflow-y: auto|scroll`）判定，**绝不看"此刻能不能滚"**（内容短时向上逃逸到祖先、让滑到最新去滚整块面板 = 「跳转范围错误」的根因），`useFollowScroll`（解析根 + window 捕获阶段 scroll 监听 + 在底跟随 + ResizeObserver 同盯内容与根）被转写尾与评论线程共用——"谁是滚动根"怎么变都不用改代码。⑦ **滚动条、内衬与截断归属**：可见条只属于贴表面边的滚动体，横向内衬由滚动体自持（评论滚动区 `.commentsScroll` 自持内衬，其内成员**不再加第二重内衬**——`.commentsScroll .interactionCard` 清掉 margin；`.interactionCard` 只留纵向 padding）；**一条表面的内容线只有一个令牌**（rail = `--dsh-tb-rail-inset`，每个成员**至多引用一次**：折叠体自带内衬时其折叠行不再补 margin，反之亦然——「上下文与运行配置」曾同时吃到 section padding 14 与按钮 margin 14 = 28px 而评论是 14px、状态行是 0px）；容器不得用 `overflow: hidden` 裁文字以外的东西，截断永远归文字子槽（`.boardStatusText` / `.chipBody` / `.sectionHead h4`）。⑧ **评论/会话面板结构（双端同一组件、同一 DOM，一切挤压/裁切/卡中部/跳转范围的根解）**：`SessionRail` 交出的不是四段而是**两块**——`.sessionRailFolds`（状态行 + 配置折叠 + 评论折叠）与调用方的 `.reviewComposer`。**宽档**：rail = flex 列 `[folds flex:1 1 auto + min-height:0] → [composer flex:none]`；folds 内 `.sessionRailHead`(**Disclosure**：展开 = **完整渲染**，无 max-height、无内部滚动条——配置裁在模型行还自带滑块正是「显示不全」的病) 与 `.sessionRailComments[data-open]`(**Disclosure**：展开 `flex:1 1 auto`+`min-height:0`，内含 `.commentsScroll` = 该档**唯一滚动体**，`scrollRef` 挂它 → 滑到最新只跳评论；折叠 `flex:none` 只留一行)。**窄档（`@container dsh-tb-panel ≤600px`）：rail 让位为 `display:contents`，`.reviewBody` 变成三行网格** `minmax(0,auto) / minmax(0,1fr) / minmax(0,auto)` = `"folds" "transcript" "composer"`——**折叠区只长到自己内容的高度（限高靠可缩不靠固定百分比：`45%` 轨在两折叠都收起时仍占掉 45% 面板高 = 对话上方一条死空带）**、**对话永远在中间弹性行**、**留言框是自己的网格行且 `min-height:0` + 自带滚动**（不再是"别人滚动内容里的最后一项"，那正是它被顶出屏幕的根因；而不可缩的 auto 轨在"软键盘 + 长草稿"时会把三轨之和顶过面板、被 `overflow:hidden` 剪掉发送按钮——恰好落在所有滚动面之外）。**谁都不许"裁了又够不着"**：每一行要么缩到 0 并自己滚，要么是中间那条 1fr。窄档两个折叠**默认收起**（对话与留言是首屏内容，用户原话），`.commentsScroll` 改 `overflow-y:visible` 把滚动让给折叠区（滚动条不套滚动条）；有挂起提问仍**强制展开**评论折叠，并且**到达即滚到该卡**（限高区视口下沿之外看不见确认/拒绝），强制展开期间折叠行真正 inert（点了没反应却偷偷翻转状态 = 表象与状态相反）。**排布由具名区域决定，绝不用 `order`**（`display:contents` 下 order 读错容器）。**rail 本身不再滚动**（`overflow: hidden`）：兜底滚动归 `.sessionRailFolds`，双端各自声明——一个含 composer 的滚动面就是"发送框被推走"的机制本身，而 `display:contents` 的 rail 声明滚动更是 `resolveScroller` 会走进去的谎。**头部**：桌面一行 = 标题+徽标 | 上下文头(flex none, ≤320px, 浮层锚头下) | 刷新/查看会话 | ×——同一基线；窄档两行 = 第 1 行 标题+徽标+×(`flex:none` 内联贴右，**绝不绝对定位**；标题单行 ellipsis，完整名进 tooltip+aria-label)，第 2 行 = 上下文头(左，`flex:1 1 auto`)+动作(右)；窄档上下文展开即**内联限高 240px**。**滚动条唯一文法** = `:is(.modalScroll,.detailBody,.commentsScroll,.reviewRail,.sessionRailFolds,.reviewTranscriptScroll,.interactionCardBody,.slashMenu,.sessionContextPanel,.refineTranscriptArea)`：`scrollbar-width:thin` + `scrollbar-color` + WebKit 8px 圆角滑块（一处列表，全板一套；**新增滚动区 = 加进这张表**）。**发送区自我说明**：排队/插话与拒绝原因的说明行归 **composer**（不是评论折叠）——折叠默认收起时它是触屏唯一可达的解释（⑩）。**高度份额不用容器/视口单位**（`cqh/cqb/vh/dvh` 禁止；`%` 只用于网格轨道这种参照为面板 definite 高度的场合）。
...
- **多端同步（host 唯一真相 + 引擎租约，一切数据一致性问题的根解）**：看板真相 = host 的 `BoardDoc`，localStorage 五键降级为离线镜像/草稿/备份。浏览器 = 乐观副本：本地写即时生效，经 `store.save → Synced*Store → BoardSyncClient` 去抖提交，**host `applyCommit` 合并后返回权威文档，所有副本向它收敛**；远端变化经 SSE（+ 轮询兜底 + 对账）→ `controller.applyRemote`（**永不回写**——回写即回声）。裁决文法见「同步域」。「添加已有会话」与侧栏拖入同走 `addTaskSource`。**引擎租约**：host 内存租约（TTL + 任何 API 命中续期 + 断流宽限期提前过期），**只有引擎端**跑 dispatch 泵/scheduler tick/reconcile/外源轮记轮/cruise 翻转/settledFollowUp+链续；非引擎端 = 纯视图 + 提交器（用户动作照常写台账，由引擎泵出；手动执行经 `requestLaunch` → host `/command` → 引擎 SSE，无引擎时 park 重放）——单泵 = 单并发预算 = 多端绝不双发（含双标签）。**席位跟可见性（双向即刻）**：`/lease` 携带 `active`，**转后台立刻写 `active:false`**（不等心跳），**可见端可从不活跃持有者抢夺席位**（双可见端不互抢、全后台维持最后持有者让定时照跑），回前台立即续租+resync——引擎永远坐在用户正看着的那台设备上。**流活性有界**：SSE 计数只在距持有者上次真实 HTTP touch `STREAM_ALIVE_MAX_MS` 内续命（半开 socket ≠ 活着）。**协议版本 `LEASE_PROTOCOL_ACTIVE` 与服务端启动时刻 `bootedAt` 随 LeaseState 下发**，且 **LeaseState 只有一处构造**（`leaseState()`；被拒分支 = `{...current, held:false}` 翻一个调用者相对的旗标）——`LeaseState` 的 `proto`/`bootedAt` 是**必填类型字段**，漏字段直接编译不过。这条是「服务端未重启」横幅说谎的根解：被拒分支曾手拼 `{held,holder,expiresAt}` 漏掉 proto，**每一个非引擎端**都从**当前**服务端读出"旧服务端"，重启多少次横幅都在（引擎端拿到正常应答故"有时显示"= 谁不是引擎谁显示）。线路上是 `LeaseWire`（旧服务端可不带，缺席本身就是证据）；**席位通知 = (held, proto, bootedAt) 元组变化**——host 重启只动协议与启动时刻不动席位，任一半移动都触发 seat 监听 → `controller.setHostProto`/`setHostBoot`（相等不 notify），横幅因此**实时清除**而非残留到一个手动刷新（只通知 held 变化 = 上一代手机横幅永不消失的第二条根因）；`hostProtoVersion()` 在首租应答前返回 **undefined**（未知 ≠ 旧，绝不误报），controller 默认取 CURRENT。旧 host（无版本）= 抢夺不生效，板头据此提示（host 改动须重启 `dsh web` 这件事界面可见）。**板头引擎指示**：同步模式且本机非引擎而有排队/待注入 → 提示"引擎在另一端"；空闲永不唠叨。**提示必须能被查证**：过旧弹窗除协议读数外还显示**该服务端进程的真实启动时间**——"我明明重启了"只有两种答案（连的确实是旧进程 / 地址指向另一个未重启实例），一个时钟读数当场判定，不用猜。**投递看门狗**：open 轮超 `OPEN_ROUND_WATCHDOG_MS` 且其会话 idle 无 turn 证据 → 合成 cancelled 结算释放（会话在跑/等人回应绝不判）；reconcile 也扫停车卡的 open 轮，`resolveCardDrop` 拒绝带 open 轮离列（僵尸轮会占并发预算并吞掉后续外源轮）。question tracker 每端独立流（先到先答，官方语义）；drafts 刻意设备本地不同步。storage hub 缺席 → 回退 localStorage 模式，功能不降级为错误。
...
- 清单：`tests/` 下 `tasks`、`schedule`、`scheduler`、`cruise`、`automation`、
  ...
  `host-sync`（同步客户端全状态机：迁移/去抖提交/对账/租约 + **席位 (held, proto, bootedAt) 元组通知**（任一半单独变化必须触发监听、首租前 proto 为 undefined）+ **双客户端共享真实
  ...
  `review-page`（review-transcript/context-meter/menu-direction/interaction 合并 + **评审家族全部 CSS 契约**：宽档 rail 两块（折叠块 flex:1 + 钉底 composer）、窄档三行网格（限高折叠区 / 对话 1fr / composer 独立行 + rail `display:contents` + 禁 order 排布）、内衬只来自 `--dsh-tb-rail-inset` 且每成员至多一次、面板容器 dsh-tb-panel、头部两行、Disclosure 文法、Dialog 正文内衬、全家族无 vh）、
  `card-contract`（card-layout + card-label 合并）、`mobile-contract`（容器查询
  机制/compact 几何无 vh/vw/触屏块/板头两行成员归属 + 确定分行 + 零段省略/会话行具名 identity 轨 + 徽标是覆盖件/一条内容线一个令牌/`<p>` 无继承间距/派生偏移/section head 双端单行/禁 auto-margin/溢出修复/离屏入口——钉死移动端设计）、
  `settings-route`、`board-route`、`board-service`（含 **LeaseState 唯一构造点机械禁令**）、`board-http`（真实
  `JsonStorageBackend` + 真实 `http.Server` + 真实 SSE 的端到端冒烟——单测 fake 测不到的
  HTTP/落盘/流式契约；含 **granted/rejected/renewing/released 四种租约应答必带 proto+bootedAt**）、
