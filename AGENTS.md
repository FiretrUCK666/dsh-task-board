...
- **车道 = 会话，卡片只是聚合（一切「为什么还在排队」的根解）**：`tasks.ts` 的 `isOpenRound` 是**唯一**的"这一轮真的在跑"判定，按种类枚举：普通轮 / refine 轮 / **外源轮（原生发言——它带 `comment` 只是留言正文，不是"排队中"）**= 未结算即在跑；评论轮 = **已注入**才在跑（保存未注入的留言不占槽、不让会话忙、不显转圈）。派生：`openRoundsOf` / `sessionIsBusy(task, sessionId)` / `hasOpenRun`（还有任何一轮在跑；**不再看"最后一条记录"**）。**并发预算数轮不数卡**（`inFlightCount`）——「并行数 3」= 三个会话同时跑；`nextEligible` 的评论/规则两车道都按**会话**取头（忙车道绝不静默旁边空闲车道）。**卡片列 = 所有会话的聚合**：`settleExecution` 仅在再无其它在跑轮时才落「待审核/待办」，否则保持「进行中」。**同一判定复用到外源检测**（`recordExternalRound`/`hasOpenRoundOn` 走 `sessionIsBusy`）——否则"会话在排队"被误当"会话在跑"，**吞掉用户在原生工作区发的消息**；反向：留言绝不插到正在跑的原生轮之前（同 lane 严格 FIFO）。**插话天生越过车道与预算**（按定义插进正在跑的对话，轮出生即 `injectedAt`，同会话两轮并存是有意不是破口）。**「排队中 · 第 N 位」数本会话车道**（`queuePositionOf` 按 `sessionId ?? sessionAnchor` 分组）。规则同走会话车道：**并行数就是节流阀**（旧的 per-card 单齐发保护改由预算承担；预算设 1 即恢复严格串行）。`tasks`/`controller`/`comment-thread` 三 spec 钉死。
...
- **清单以 `tests/` 目录为准**（一名一个 spec，命名即被测模块/表面）。需要知道的非显然分工：
  `board-doc` = 同步合并文法；`host-sync` = 同步客户端全状态机（迁移/去抖/对账/租约 +
  **席位 (held, proto, bootedAt) 元组任一半变化都要触发监听**、首租前 proto 为 undefined +
  **双客户端共享真实 `BoardDataService` 的端到端收敛**）；`question-tracker` = mux 问答流自愈
  （失败/关闭必重连、dispose 后必不重连）；`transcript-cache` = 读取新鲜度层（在途合并/TTL/
  超时/失败不缓存）；`board-service` 含 **LeaseState 唯一构造点的机械禁令**，`board-http` 用真实
  存储后端 + 真实 http.Server + 真实 SSE 覆盖单测 fake 测不到的链路（含**四种租约应答必带
  proto+bootedAt**）；`controller` 是唯一大文件（端到端，共享 harness 不拆分）；
  `review-page` / `mobile-contract` / `card-contract` 承载**全部 CSS 布局契约**（见上文设计系统层）；
