...
- **浮层几何（看板盒参照，全宽度统一）**：`.modalBackdrop` = 看板盒内 absolute inset:0 + `overflow:auto` + `margin:auto` 子项居中（**不用 align-items:center**——超高面板顶部会被裁到滚动之外），底部内边距携 `--dsh-tb-kb`（键盘内缩单一变量，见 keyboard-inset.ts——浮层随剩余空间收身，标题/动作永不被键盘顶出屏幕）。**浮层家族（`.modal/.detail/.review/.presetModal/.autoModal`）宽高一律板盒 % 参照，禁 vw/vh**（确需视口参照才用 `dvh`）。**嵌套浮层必须 portal 到看板盒**（`Dialog` 的 `portal` prop；`ConfirmDialog`/`SessionFrame` 恒 portal——后者是评论面板外壳，曾被宿主 `.detail` 的 relative+overflow:hidden 锚定裁剪成「弹不出来」）——内层留在宿主 DOM 必被其锚定并裁剪。**每个弹窗正文必须是 `.modalScroll` 唯一滚动体**（底部动作钉在滚动体外永远可达；正文裸露堆叠即「底部被截」根因）。窄屏（`useNarrow()`）下锚定弹层换形态：巡航设置用 `Dialog`（弹层几何在小板盒上不可靠），宽屏保持 popover。
...
- **动效唯一语法**：注意力 = `--dsh-tb-attention` + `--dsh-tb-breath`；卡片外层呼吸环（`.card[data-unviewed]`/`.card[data-active]`）、会话行内层柔晕（`data-glow` 绑定、pause 而非 cancelled——永不闪烁）、浮层入场 `dshTbDialogIn`；`prefers-reduced-motion` 全部静态降级（手机系统开了动画缩减则全静默属标准行为，不是 bug）。
...
- **响应式与触屏（改窄屏/触摸问题先读这里）**：① **单一响应式参照 = 板盒宽，不用视口**——`[data-dsh-taskboard-view]` 设 `container-type: inline-size` + `container-name: dsh-tb`（并带 `-webkit-text-size-adjust:100%` 抗安卓/微信 font boosting），几何断点全部写进 `@container dsh-tb (max-width: …)`（compact 档 680px、review 堆叠 719px）；侧栏折叠/分屏/手机都自动正确，**板体布局永不用 `@media (max-width)`**。② **compact 列 = 横向滑轨**：`flex: 0 0 clamp(200px, 46cqw, 320px)`（约两列同屏、邻列露边暗示可滑）+ scroll-snap + **列导航 tab**（`.columnTabs` 基显 none、compact 显现——列头文法原样：状态点+名+计数，点按 `scrollIntoView` 直达、滚动位置回写高亮，`activeColumnIndexAt` 纯函数）；compact 底 `env(safe-area-inset-bottom)`。③ **触屏 = 桌面 parity**：交互一件不改（悬停显露保持悬停显露、拖拽保持拖拽、不添加任何按钮副路径），`@media (hover: none) and (pointer: coarse)` 块**只承载隐形人体工学**——小控件 `::before` 透明热区扩到 ≥36px（视觉 28/24 节奏不破）、色点 swatch 24px、输入 `font-size:16px`（防聚焦自动缩放）。④ 行为/结构开关只认 `useNarrow()`（matchMedia 单一信号，用于弹层换形态这类 CSS 无法表达的分支）。`tests/mobile-contract.spec.ts` 钉死以上全部。
...
- **运行态与会话集合唯一推导** = `task-live.ts` + `linked-sessions.ts`：`relatedSessionIdsOf(task, linkedIds)`（refine → 显式 session 绑定 → 执行轮 → 注入 linked，去重稳定序；一切相关面只经此推导）；`taskLiveStateOf`（waiting > running > idle，running 以相关会话原生 `byId.running` 为真相）。`deriveLinkedSessions`：**链接行只来自显式 session 绑定**——工作区绑定是来源/配置关联，**永不派生成员会话**（主界面新建的会话绝不会自己进卡片；此为有意的产品语义，勿"恢复"）。
...
- **同步域（多端一致的地基）**：`board-doc.ts`（**BoardDoc + 合并文法唯一居所**：逐记录以**作者声明**裁决——commit 携 `changed`（本副本相对上次同步基线内容移动过的 id 集，由 `changedIdsOf` 对当时视图 diff），claimed 记录无条件接收（host 串行到达序 = 后到者胜，**设备时钟不参与裁决**；内容相等的 claim 是 no-op），未 claimed 维持 LWW（陈旧副本绝不覆盖新编辑），删除带 `baseUpdatedAt` + 墓碑（at=所见最新+1，抗偏移压复活、真更新可复活），`stamps` 记 host 接收钟，section LWW（at>= 覆盖）；`normalizeBoardDoc`/`normalizeCruiseValue` 把介质的值当数据不当真相）· `host-sync.ts`（`BoardSyncClient`：boot 一次性迁移〔host 空 → 整视图 bootstrap；host 非空且本地分歧 → 按记录联合并入、共享 section 归首写者、分歧整视图 `onBackup` 停放〕、claims 在 `setTasks` 逐次累积 / 全量 ack 后清空（陈旧全数组快照永不误 claim 回退远端编辑）、去抖提交（在途合并 / trailing refire / 失败退避）、SSE + 轮询 + 重连汇入单一 resync、引擎租约心跳（任何 API 命中即续期）、`requestLaunch` 中继；`Synced*Store` = 既有 store 接缝 over 共享文档 + 本地镜像）。

### 关键不变量（避免重造已有机制；改前先读对应文件）

- **多端同步（host 唯一真相 + 引擎租约，一切数据一致性问题的根解）**：看板真相 = host 的 `BoardDoc`（storage hub json 后端持久化），localStorage 五键**降级为离线镜像/草稿/备份**（键名不变）。浏览器 = 乐观副本：本地写即时生效，经 `store.save → Synced*Store → BoardSyncClient` 去抖提交，**host `applyCommit` 合并后返回权威文档，所有副本向它收敛（无 CAS 也无不收敛）**；远端变化经 SSE（+ 轮询兜底 + 重连/回前台对账）→ `controller.applyRemote`（**永不回写**——回写即回声）。裁决文法见「同步域」：**作者声明（changed）优先、LWW 兜底、墓碑压复活**——设备时钟好坏不影响"谁的最近操作生效"（会话拖拽排序等任意字段两端必达）。「添加已有会话」与侧栏拖入同走 `addTaskSource`。**引擎租约**：host 内存租约（TTL + 任何 API 命中续期 + SSE 断流宽限期提前过期），**只有引擎端**跑 dispatch 泵/scheduler tick/reconcile/外源轮记轮/cruise 翻转/settledFollowUp+链续；非引擎端 = 纯视图 + 提交器，用户动作照常写台账（同步后由引擎泵出），手动执行经 `requestLaunch` → host `/command` → 引擎 SSE 执行（无引擎时 park，下次授约重放）——单泵 = 单并发预算 = 多端绝不双发（含同浏览器双标签）。question tracker 每端独立流（先到先答，官方语义）；drafts 刻意设备本地不同步。storage hub 缺席 → 整体回退 localStorage 模式 = 旧行为，功能不降级为错误。
