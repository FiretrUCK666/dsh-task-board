...
- `src/index.ts`：`inject = ['webServer','systemPrompt','settings']`；注册设置命名空间（settings.yaml 持久化）、设置/权限/看板/附件/会话状态路由，公告与 systemPrompt section 随开关联动。
...
- **响应式与触屏（改窄屏/触摸问题先读这里）**：① **单一响应式参照 = 板盒宽，不用视口**——`[data-dsh-taskboard-view]` 设 `container-type: inline-size` + `container-name: dsh-tb`（并带 `-webkit-text-size-adjust:100%` 抗安卓/微信 font boosting），几何断点全部写进 `@container dsh-tb (max-width: …)`（compact 档 680px、review 堆叠 600px=双列真实下包络：弹性 rail `min(344px,46cqw)` + 转写最小可读 + 内缩）；侧栏折叠/分屏/手机都自动正确，**板体布局永不用 `@media (max-width)`**。② **compact 列 = 自由横向滑轨**：`flex: 0 0 clamp(200px, 46cqw, 320px)`（约两列同屏、邻列露边暗示可滑）+ **列导航 tab**（`.columnTabs` 基显 none、compact 显现——列头文法原样：状态点+名+计数，点按 `scrollIntoView` 直达、滚动位置回写高亮，`activeColumnIndexAt` 纯函数）；**轨道不用 scroll-snap**（snap 在容器宽度变化时按 px scrollLeft 重吸附→开合侧栏看板每次"往右偏一点"累积漂移；桌面无 snap=parity，直达交给 tab）；compact 底 `env(safe-area-inset-bottom)`。③ **触屏 = 桌面 parity**：交互一件不改（悬停显露保持悬停显露、拖拽保持拖拽、不添加任何按钮副路径），`@media (hover: none) and (pointer: coarse)` 块**只承载隐形人体工学**——小控件 `::before` 透明热区扩到 ≥36px（视觉 28/24 节奏不破）、色点 swatch 24px、输入 `font-size:16px`（防聚焦自动缩放）。④ 行为/结构开关只认 `useNarrow()`（matchMedia 单一信号，用于弹层换形态这类 CSS 无法表达的分支）。⑤ **会话行 = 具名 grid**（`.sessionRowTop` 槽位 lead/chip/act，compact 重排两行 `"lead act"/"chip act"`）——chip 与动作在每行同一 x、永不随标题长度换行漂移（内容驱动 flex-wrap 是"排版乱、没对齐"根因）。`tests/mobile-contract.spec.ts` 钉死以上全部。
...
- **同步域（多端一致的地基）**：`board-doc.ts`（**BoardDoc + 合并文法唯一居所**：逐记录以**作者声明**裁决——commit 携 `changed`（本副本相对上次同步基线内容移动过的 id 集，由 `changedIdsOf` 对当时视图 diff），claimed 记录无条件接收（host 串行到达序 = 后到者胜，**设备时钟不参与裁决**；内容相等的 claim 是 no-op），未 claimed 维持 LWW（陈旧副本绝不覆盖新编辑），删除带 `baseUpdatedAt` + 墓碑（at=所见最新+1，抗偏移压复活、真更新可复活），`stamps` 记 host 接收钟；section 层同构 claim——commit 携 `sectionClaims`（本副本 dirty 的 section），claimed section 无条件接收 + host 时钟落 at、未 claim 的基线副本一律跳过（陈旧 at 绝不再覆盖），无 `sectionClaims` 字段的旧客户端回落纯 LWW；`normalizeBoardDoc`/`normalizeCruiseValue` 把介质的值当数据不当真相）· `host-sync.ts`（`BoardSyncClient`：boot 一次性迁移〔host 空 → 整视图 bootstrap；host 非空且本地分歧 → 按记录联合并入、共享 section 归首写者、分歧整视图 `onBackup` 停放〕、claims 在 `setTasks` 逐次累积 / 全量 ack 后清空（陈旧全数组快照永不误 claim 回退远端编辑）、去抖提交（在途合并 / trailing refire / 失败退避）、SSE + 轮询 + 重连汇入单一 resync、引擎租约心跳（任何 API 命中即续期，携带 `active`=本 tab 可见性）、`requestLaunch` 中继；`Synced*Store` = 既有 store 接缝 over 共享文档 + 本地镜像）。**用户意图写必经 `controller.userEdit(taskId, mutate)` 单一漏斗**（返回新对象即结构上强制 bump `updatedAt`——隐藏/删除/规则开关/排序等 8 处"静默吞写"根因灭绝；引擎派生写与读态写 viewedAt 不走此漏斗，各有自己的新鲜度语义；排序被挤位的 sibling 也补 stamp）。

### 关键不变量（避免重造已有机制；改前先读对应文件）

- **多端同步（host 唯一真相 + 引擎租约，一切数据一致性问题的根解）**：看板真相 = host 的 `BoardDoc`（storage hub json 后端持久化），localStorage 五键**降级为离线镜像/草稿/备份**（键名不变）。浏览器 = 乐观副本：本地写即时生效，经 `store.save → Synced*Store → BoardSyncClient` 去抖提交，**host `applyCommit` 合并后返回权威文档，所有副本向它收敛（无 CAS 也无不收敛）**；远端变化经 SSE（+ 轮询兜底 + 重连/回前台对账）→ `controller.applyRemote`（**永不回写**——回写即回声）。裁决文法见「同步域」：**作者声明（changed）优先、LWW 兜底、墓碑压复活**——设备时钟好坏不影响"谁的最近操作生效"（会话拖拽排序等任意字段两端必达）。「添加已有会话」与侧栏拖入同走 `addTaskSource`。**引擎租约**：host 内存租约（TTL + 任何 API 命中续期 + SSE 断流宽限期提前过期），**只有引擎端**跑 dispatch 泵/scheduler tick/reconcile/外源轮记轮/cruise 翻转/settledFollowUp+链续；非引擎端 = 纯视图 + 提交器，用户动作照常写台账（同步后由引擎泵出），手动执行经 `requestLaunch` → host `/command` → 引擎 SSE 执行（无引擎时 park，下次授约重放）——单泵 = 单并发预算 = 多端绝不双发（含同浏览器双标签）。**席位跟可见性**：`/lease` 携带 `active`（本 tab `visibilityState`），**可见端可从不活跃持有者抢夺席位**（SSE 广播旧持有者即时降级、新引擎上任即 reconcile 补记漏轮；双可见端不互抢、全后台维持最后持有者让定时照跑），回前台立即续租+resync（不等心跳）——引擎永远坐在用户正看着的那台设备上（"手机点开始没反应、电脑端才动、刷新才好"根因）。question tracker 每端独立流（先到先答，官方语义）；drafts 刻意设备本地不同步。storage hub 缺席 → 整体回退 localStorage 模式 = 旧行为，功能不降级为错误。
- **统一会话与评论单轨**：相同会话 = 同一条线程（`sessionCommentsOf`）；直发/驱动/评论并轨为一种留言；发送两态 = 排队（调度器/巡航/FIFO，可取消）与插话（`steerComment` 立即送达）；图片走 `AttachmentStrip` → host 附件桥 → prompt part（`{type:'image', attachment:{attachmentId,mediaType}}` 与原生 composer 同形）——桥体上限对齐原生聚合（`readJsonBodyDetailed` 区分超大 413/损坏 400，非旧的 1MB 默认静默吞），`admitDraftImages` 返回 `{refs,error}` 透出原因、composer 就近 `.formError` 展示（绝不静默丢图），附件条独立成行、动作行恒单行（发送按钮永不被 chips 挤没）。
...
- **会话标题（与原生一轨，绝不打架）**：原生 rename = 官方「钉住」语义（`sessions.rename` 以 `user` 源追加 `session/title`——钉住标题、作废在途自动生成、后续消息永不自动改名；不命名则宿主自动命名链完整保留：首条消息 → 确定性 fallback + provider cadence）。板内两条入口都走同一官方写：**新建会话的标题选填**（`createTaskSession` 的 `title`——填了走官方 rename 钉住，留空什么都不发、交给宿主自动命名，绝不制造占位名与原生打架；失败 = `titleError` 部分成功）；**会话行重命名**（`SessionRow` 行内铅笔 → `renameTaskSession`，目标必须是本任务相关会话，空标题拒绝，成功只 notify——标题真相已在宿主 list，无第二份状态）。**未命名文法**：宿主未命名的会话行统一显示 `detail.sessionUntitled`（`sessionRowTitleOf` 唯一推导 + `untitledLabel` 注入，run 行与 id 命名的 linked 行都覆盖——绝不回退任务标题或裸 id）。**认得宿主的确定性自动名**：`controller.sessionTitle` 一处判据——durable `title` 恰等于会话 `cwd` 的 project basename（含 Windows/POSIX 分隔与尾斜杠）即宿主首条消息后的 fallback 名、非真名，返回 undefined 走未命名文法，直到 provider 生成名或用户 rename 钉住（"不填标题却显示工作区名"多次未修好的根因；用户故意把工作区名当标题为可接受的唯一歧义边界）。
...
- 清单：`tests/` 下 `tasks`、`schedule`、`scheduler`、`cruise`、`automation`、
  ...
  惯用法各占一个）、`route-scope`（client 设置 scope）、`transcript-cache`（转写读取新鲜度层：在途合并/TTL/超时/失败不缓存）、`controller`（端到端，
