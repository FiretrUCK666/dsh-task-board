...
- **详情页新建会话（会话区「+ 新建会话」）**：`Section` 的可选 `action` 槽（`.sectionHead` 标题行文法）承载入口；`NewSessionModal` = `Dialog portal`（嵌套弹层硬性规范）+ `.modalScroll` 唯一滚动体，正文只有运行配置（初始值 = 任务自身配置）。链路 = `controller.createTaskSession` → `ExecutionService.createSession`（**保证全新会话**：client `sessions.create` face 结构化读取 + wire `sessions.create` 兜底；face 缺失降级 connectWorkspace blank 复用）→ 按执行同序应用配置（model → agentPreset → `/permission`）→ `addTaskSource` 绑定（多源叠加/持久化/即时 reconcile 全复用）。**失败契约**：创建失败 = ok:false、任务不动；配置失败（会话已在）= 诚实部分成功 `configError`，会话保留、弹层内联展示——绝不静默吞掉，也绝不回滚已创建的会话。新会话不是执行：无执行轮、不进调度器、不涉自动化。
